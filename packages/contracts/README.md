...
defines the Connect app control plane, `briar.sidecar.v1` defines the framed
runner protocol, `briar.realtime.v1` defines WebSocket frames, and
`briar.types.v1` contains the small set of values truly shared by those
boundaries. Effect schemas remain responsible for domain invariants and
cross-field rules that protobuf cannot express.
