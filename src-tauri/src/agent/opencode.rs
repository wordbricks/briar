...
impl OpenCodeRuntime {
    pub(crate) fn discover(
        ...
        let bun_binary = command_runner.resolve_binary("bun").map_err(|_| {
            "OpenCode runner 실행에 필요한 Bun을 로컬 환경에서 찾지 못했습니다.".to_string()
        })?;
        let opencode_binary = command_runner.resolve_binary("opencode")?;
    ...
    #[cfg(test)]
    fn for_test(bun_binary: PathBuf, opencode_binary: PathBuf, runner: PathBuf) -> Self {
        ...
        Self {
            ...
            opencode_binary: opencode_binary.to_string_lossy().into_owned(),
            runner,
        }
    }
}
...
impl OpenCodeConnection {
    fn start(
        ...
        let mut child = runtime
            .command_runner
            .spawn_piped(&spec)
            .map_err(|error| format!("OpenCode runner를 시작하지 못했습니다: {error}"))?;
        let stdin = child
            .stdin
            .take()
            .ok_or_else(|| "OpenCode runner 입력을 열지 못했습니다.".to_string())?;
        let stdout = child
            .stdout
            .take()
            .ok_or_else(|| "OpenCode runner 출력을 열지 못했습니다.".to_string())?;
        let mut child_stderr = child
            .stderr
            .take()
            .ok_or_else(|| "OpenCode runner 오류 출력을 열지 못했습니다.".to_string())?;
    ...
    fn send(&mut self, message: &Value) -> Result<(), String> {
        serde_json::to_writer(&mut self.stdin, message)
            .map_err(|error| format!("OpenCode runner 요청을 만들지 못했습니다: {error}"))?;
        self.stdin
            .write_all(b"\n")
            .and_then(|_| self.stdin.flush())
            .map_err(|error| format!("OpenCode runner에 요청을 보내지 못했습니다: {error}"))
    }

    fn read(&mut self) -> Result<Option<OpenCodeRunnerMessage>, String> {
        let mut line = String::new();
        let bytes = self
            .stdout
            .read_line(&mut line)
            .map_err(|error| format!("OpenCode runner 응답을 읽지 못했습니다: {error}"))?;
        if bytes == 0 {
            return Ok(None);
        }
        serde_json::from_str(&line)
            .map(Some)
            .map_err(|error| format!("OpenCode runner가 잘못된 응답을 보냈습니다: {error}"))
    }
    ...
    fn exit_error(&mut self) -> String {
        ...
        let stderr = self
            ...
            .unwrap_or_default();
        if stderr.is_empty() {
            "OpenCode runner가 결과를 반환하지 않고 종료되었습니다.".to_string()
        } else {
            format!("OpenCode runner가 종료되었습니다: {stderr}")
        }
    }
}
...
pub(crate) fn chat(
    ...
    let runner_request = OpenCodeRunnerRequest {
        ...
        opencode_binary: &runtime.opencode_binary,
    };
    let raw_request = serde_json::to_value(&runner_request)
        .map_err(|error| format!("OpenCode 요청을 만들지 못했습니다: {error}"))?;
    ...
    let mut connection =
        OpenCodeConnection::start(runtime, &workspace_root, &execution.environment)?;
    connection.send(&raw_request)?;
    loop {
        match connection.read()? {
            Some(OpenCodeRunnerMessage::Session { session_id }) => {
                if session_id.trim().is_empty() {
                    return Err("OpenCode가 빈 대화 ID를 반환했습니다.".to_string());
                }
                if let Some(event_sink) = execution.event_sink.as_ref() {
            ...
            Some(OpenCodeRunnerMessage::Event { raw, event }) => {
                if let Some(event_sink) = execution.event_sink.as_ref() {
                    event_sink(AgentProviderEvent {
                        provider: AgentProviderKind::Opencode,
                        direction: AgentEventDirection::Server,
            ...
            Some(OpenCodeRunnerMessage::Approval {
                id,
                tool_name,
                input,
                title,
            ...
            Some(OpenCodeRunnerMessage::Result {
                session_id,
                message,
            }) => {
                if session_id.trim().is_empty() {
                    return Err("OpenCode가 대화 ID를 반환하지 않았습니다.".to_string());
                }
            ...
            Some(OpenCodeRunnerMessage::Error { message }) => {
                return Err(format!("OpenCode 요청에 실패했습니다: {message}"));
            }
...
fn encode_conversation_id(project_id: &str, session_id: &str) -> String {
    format!("briar:opencode:{project_id}:{session_id}")
}
...
fn decode_conversation_id<'a>(
    ...
    let prefix = format!("briar:opencode:{project_id}:");
    conversation_id
        .strip_prefix(&prefix)
...
mod tests {
    ...
    #[cfg(unix)]
    #[test]
    fn runs_the_opencode_runner_and_maps_events_and_approvals() {
...
"#,
    ...
    #[test]
    fn scopes_conversation_ids_to_opencode_and_the_project() {
        assert_eq!(
            decode_conversation_id("project-1", "briar:opencode:project-1:session-1"),
            Ok("session-1")
        );
        assert!(decode_conversation_id("project-2", "briar:opencode:project-1:session-1").is_err());
    }
}
