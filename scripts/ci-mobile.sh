...
readonly swift_project="ios/BriarCompanion/BriarCompanion.xcodeproj"
...
rg -F 'kSecAttrAccessibleAfterFirstUnlockThisDeviceOnly' ios/BriarCompanion/App/SessionStore.swift >/dev/null
rg -F 'session.download(for: request)' ios/BriarCompanion/App/MobileAPIContract.swift >/dev/null
if rg -n '(^|[^A-Za-z])(print|debugPrint|dump)\(' ios/BriarCompanion/App; then
...
bun run tauri ios build --debug --target aarch64-sim --ci --no-sign
