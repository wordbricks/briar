...
if [[ "${BRIAR_TRUSTED_MERGE_GROUP_CI:-}" == "1" ]]; then
  ...
  [[ -r "$expected_lock" ]] || {
    echo "[local-ci] Infrastructure: executor lacks audited Bun lock proof." >&2
    exit 75
  }
  sha256sum --check --status "$expected_lock" || {
    echo "Candidate bun.lock differs from the image's online-audited lock." >&2
    exit 1
  }
  echo "Verified bun.lock against the online audit baked into the OCI image."
else
