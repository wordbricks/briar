...
function ProductStage({ c }: { c: LandingCopy }) {
  return (
    <div className="product-stage" aria-label={c.aria.productPreview}>
      ...
      <div className="real-product-screen">
        <img
          src="/briar-dashboard.jpeg"
          alt={c.aria.productPreview}
          width="1231"
          height="768"
        />
      </div>
...
function WorkflowVisual({ c }: { c: LandingCopy }) {
  return (
    <div
      className="workflow-visual real-product-screen"
      aria-label={c.workflow.title}
    >
      <img
        src="/briar-evidence.jpeg"
        alt={`${c.workflow.title} — ${c.workflow.issueTitle}`}
        width="1231"
        height="768"
      />
    </div>
