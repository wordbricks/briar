...
describe("AgentUsageStatusBar", () => {
  ...
  it("shows only enabled providers and uses logo-only status for missing usage", async () => {
    localStorage.setItem("briar.locale.v1", "en");
    const container = document.createElement("div");
    document.body.append(container);
    const root = createRoot(container);
    ...
    expect(
      trigger?.querySelectorAll(".agent-usage-status-provider"),
    ).toHaveLength(2);
    expect(trigger?.querySelectorAll(".agent-usage-provider-icon")).toHaveLength(
      2,
    );
    expect(trigger?.textContent).toContain("48%");
    expect(trigger?.textContent).not.toContain("OpenCode");
    expect(
      trigger?.querySelector('[role="img"][aria-label="OpenCode"]'),
    ).not.toBeNull();
    expect(trigger?.textContent).not.toContain("Codex");
    expect(trigger?.textContent).not.toContain("Grok");

    await act(async () => trigger?.click());
    const roster = container.querySelectorAll(".agent-usage-provider-row");
