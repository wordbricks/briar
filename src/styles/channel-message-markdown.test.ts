...
describe("channel message markdown presentation", () => {
  it("keeps links and inline code visually distinct", () => {
    const link = ruleBody(".channel-message-text a:not(.channel-mention-button)");
    expect(link).toContain("text-decoration:underline");
    expect(link).toContain("text-underline-offset:2px");

    const inlineCode = ruleBody(".channel-message-text code");
