...
type Listener = (frame: IssueAgentActivityFrame) => void;
...
export class IssueActivityRealtimeTransport {
  ...
  stop() {
    ...
    this.socket = null;
    socket?.close(1000, "Issue activity stopped");
  }

  private async connect(generation: number) {
    try {
      const fetchImpl = this.input.fetch ?? fetch;
      const path = `/projects/${this.input.projectId}/runs/${this.input.runId}/` +
      ...
      if (!response.ok) {
        throw new Error(`Issue activity ticket failed (${response.status})`);
      }
      const body = await response.json() as { url?: unknown };
      if (typeof body.url !== "string" || !/^wss?:\/\//u.test(body.url)) {
        throw new Error("Issue activity ticket returned an invalid URL");
      }
      ...
      socket.addEventListener("error", () => {
        socket.close(1011, "Issue activity socket failed");
        finish();
      });
    } catch (error) {
      console.warn("Issue activity socket disconnected", error);
      if (this.active && generation === this.generation) {
        this.scheduleReconnect(generation);
      }
    }
