  ...
  ko: {
    ...
    entries: [
      ...
      },
      {
        ...
        summary:
          "실행 Worker가 폴링 대신 소켓으로 즉시 깨어나 DM 답장 대기가 중앙값 14초에서 사라졌고, 짧은 메시지를 연달아 보내도 하나의 대화로 묶여 답장이 한 번만 옵니다. 화면이 필요 없는 작업이 Computer Use 설정 때문에 큐에 영구히 남던 문제도 고쳤습니다.",
        items: [
          "메시지가 큐에 들어가면 실행 Worker를 소켓으로 즉시 깨웁니다. 이전에는 빈 조회 후 15초를 자고 최대 60초까지 물러났기 때문에 큐에 들어간 시점과 작업이 시작된 시점 사이에 중앙값 14초, 최대 약 32초의 공백이 있었습니다.",
          "DM에서 짧은 메시지를 연달아 보내면 하나의 대화로 묶여 한 번만 답합니다. 이전에는 메시지마다 별도 세션이 생겨 거의 같은 답장이 여러 번 왔습니다. 채널 스레드 동작은 그대로입니다.",
          "Computer Use 권한이 켜진 에이전트가 화면 없는 호스트에서도 실행됩니다. 권한을 호스트 요구 조건으로 잘못 해석해 화면이 전혀 필요 없는 작업까지 영구히 큐에 남던 문제를 고쳤습니다.",
        ],
      },
      {
        version: "1.2.210",
