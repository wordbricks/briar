...
import {
  registerWorkspaceAgentContextService,
  type WorkerConnectWorkspaceContextServices,
} from "./worker-connect-workspace-context";
...
const services = (): WorkerConnectWorkspaceContextServices => ({
  ...
  getWorkspaceProject: vi.fn().mockResolvedValue({ id: projectId }),
  ...
  lookup: vi.fn<WorkerConnectWorkspaceContextServices["lookup"]>()
...
const invoke = async (
  ...
  body: unknown,
  dependencies: WorkerConnectWorkspaceContextServices,
) => {
...
describe("Workspace Agent context Connect adapter", () => {
  it("returns a typed manifest or an explicit unchanged result", async () => {
    ...
    expect(unchanged.result).toMatchObject({
      case: "unchanged",
      value: { workspaceId: workspaceId, workId, revision },
    });
  });
  ...
  it("maps every generated lookup oneof exactly once and preserves ordering", async () => {
    ...
    expect(dependencies.getWorkspaceProject).toHaveBeenCalledOnce();
