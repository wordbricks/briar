...
export async function getTeamSettings(db: D1Database, projectId: string) {
  return await db
    .prepare(
    ...
    .bind(projectId)
    .first<TeamSettingsRow>();
}

export async function updateTeamMandatoryCheckpoints(
  db: D1Database,
  projectId: string,
...
export async function updateTeamSettings(
  ...
  await db
    ...
    .run();
  return await getTeamSettings(db, projectId);
}
