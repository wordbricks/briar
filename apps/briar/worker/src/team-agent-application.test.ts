...
import {
  type TeamAgentApplicationServices,
...
const existing: TeamAgentRow & {
  skills: NonNullable<TeamAgentRow["skills"]>;
} = {
...
const write = decodeTeamAgentInput({
  name: "Builder",
...
const setup = () => {
  ...
  const updateAgent = vi.fn<TeamAgentApplicationServices["updateAgent"]>();
  const services: TeamAgentApplicationServices = {
    createAgent: vi.fn(),
    deleteAgent: vi.fn(),
...
describe("project Agent application", () => {
  ...
  it("removes the replaced Codex Pet only after persistence succeeds", async () => {
    ...
    const updated = await updateTeamAgentApplication(
      {
        db: {} as D1Database,
        attachmentsBucket,
        project,
