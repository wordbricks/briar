...
describe("project Agent session storage cutover", () => {
  it("backfills visible history and rejects corrupt storage", async () => {
    ...
    await applyD1Migrations(db, {
      through: "0168_canonical_agent_execution_metrics_storage.sql",
    });
    ...
    const migratedPayload = decodeStoredTeamAgentSessionPayload(
      (await db.prepare(
    ...
    const migratedSummaries = await listTeamAgentSessionSummaries(
      db,
      projectId,
    ...
    expect(migratedSummaries.map((row) => ({
      id: row.session_id,
      value: decodeStoredTeamAgentSessionSummary(row.summary_json),
    ...
    const changes = await listTeamAgentSessionChanges(db, projectId, 0);
    ...
    const stored = await upsertTeamAgentSession(db, {
      projectId,
      id: sessionId,
      requestedByUserId: "session-owner",
      payload: {
    ...
    const payload = decodeStoredTeamAgentSessionPayload(stored!.payload_json);
    expect(payload).toMatchObject({
      requestedByUserId: "session-owner",
      summary: "S".repeat(2_500),
    });

    const [summaryRow] = await listTeamAgentSessionSummaries(
      db,
      projectId,
      [sessionId],
      "session-owner",
    );
    const summary = decodeStoredTeamAgentSessionSummary(
