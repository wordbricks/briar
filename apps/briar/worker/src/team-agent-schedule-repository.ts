...
import {
  type TeamAgentScheduleRow,
  type TeamAgentScheduleRunRow,
  type TeamAgentScheduleRunStatus,
...
export async function listTeamAgentSchedules(
  db: D1Database,
  projectId: string,
) {
  const result = await db
    ...
    .bind(projectId)
    .all<TeamAgentScheduleRow>();
  return result.results;
}

export async function getTeamAgentScheduleCreatorId(
  db: D1Database,
  projectId: string,
  scheduleId: string,
) {
...
export async function createTeamAgentSchedule(
  db: D1Database,
  projectId: string,
  input: TeamAgentScheduleInput,
) {
  ...
  return await db
    ...
    .bind(id, projectId)
    .first<TeamAgentScheduleRow>();
}

export async function updateTeamAgentSchedule(
  db: D1Database,
  projectId: string,
  scheduleId: string,
  input: TeamAgentScheduleInput,
...
export async function deleteTeamAgentSchedule(
  db: D1Database,
  projectId: string,
  scheduleId: string,
...
type UnhydratedTeamAgentScheduleRunRow = Omit<
...
async function hydrateScheduleRunAgentSkills(
  db: D1Database,
  rows: readonly UnhydratedTeamAgentScheduleRunRow[],
): Promise<TeamAgentScheduleRunRow[]> {
...
export async function listTeamAgentScheduleRuns(
  db: D1Database,
  projectId: string,
) {
  const result = await db
    ...
    .bind(projectId)
    .all<UnhydratedTeamAgentScheduleRunRow>();
...
async function reclaimExpiredProjectAgentScheduleRun(
  ...
  const selected = await db
    ...
    .first<UnhydratedTeamAgentScheduleRunRow>();
  if (!selected) return null;
  return (await hydrateScheduleRunAgentSkills(db, [selected]))[0];
}

export async function listClaimableTeamAgentScheduleTeamIds(
  db: D1Database,
  userId: string,
...
export async function claimDueTeamAgentScheduleRun(
  db: D1Database,
  projectId: string,
  input: {
    claimTokenHash: string;
...
export async function completeTeamAgentScheduleRun(
  db: D1Database,
  projectId: string,
  runId: string,
  input: {
    claimTokenHash: string;
    status: Exclude<TeamAgentScheduleRunStatus, "running">;
...
export async function renewTeamAgentScheduleRunLease(
  db: D1Database,
  projectId: string,
  runId: string,
  input: {
