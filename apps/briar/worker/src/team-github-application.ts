...
export type TeamGithubAccess = {
  readonly id: string;
  readonly organization_id: string;
};

export async function requireTeamGithubAccess(input: {
  auth: BriarAuth;
  db: D1Database;
  request: Request;
  projectId: string;
}): Promise<TeamGithubAccess> {
  ...
  const project = await input.db
    ...
    .first<TeamGithubAccess>();
  if (!project) throw new HttpError(404, "Project not found");
  return project;
}

export async function teamGithubIdentity(
  db: D1Database,
  project: TeamGithubAccess,
...
type TeamGithubApplicationInput = {
  readonly db: D1Database;
  readonly env: Env;
  readonly project: TeamGithubAccess;
};

export async function getTeamMergeActivityApplication(input: {
  db: D1Database;
  env: Env;
  projectId: string;
  userId: string;
  ...
  if (!project) throw new HttpError(404, "Project not found");
  const identity = await teamGithubIdentity(input.db, project);
  return githubAppApiOperation(() => getProjectGithubMergeActivity(input.env, identity));
}

export async function createTeamGithubCredentialApplication(
  input: TeamGithubApplicationInput,
) {
  const identity = await teamGithubIdentity(input.db, input.project);
...
export async function getTeamGithubRepositoryApplication(
  input: TeamGithubApplicationInput,
) {
  const identity = await teamGithubIdentity(input.db, input.project);
...
export async function createTeamGithubPullRequestApplication(
  input: TeamGithubApplicationInput & { request: unknown },
) {
...
export async function getTeamGithubPullRequestApplication(
  input: TeamGithubApplicationInput & { pullRequestNumber: number },
) {
  const identity = await teamGithubIdentity(input.db, input.project);
  return githubAppApiOperation(() =>
    getProjectGithubPullRequest(
...
export async function updateTeamGithubPullRequestApplication(
  input: TeamGithubApplicationInput & {
...
export async function mergeTeamGithubPullRequestApplication(
  input: TeamGithubApplicationInput & {
    pullRequestNumber: number;
    request: unknown;
  },
...
export async function createTeamGithubCommitStatusApplication(
  input: TeamGithubApplicationInput & { request: unknown },
) {
