...
import { toConnectError } from "./mobile-connect-errors";

export type MobileConnectChannelInput = {
...
export type MobileConnectChannelServices = {
...
const mobileConnectChannelServices: MobileConnectChannelServices = {
...
const sourceWithChannelOneofs = (
  ...
  switch (descriptor.typeName) {
    case "briar.mobile.v1.ChannelSummary":
      adapted.directMessageParticipants = source.dmParticipants;
      break;
    case "briar.mobile.v1.DirectMessageParticipant":
    case "briar.mobile.v1.ChannelMessageAuthor":
      adapted.kind = source.type;
      break;
    case "briar.mobile.v1.BlockText":
      adapted.kind = source.type === "mrkdwn" ? "markdown" : source.type;
      break;
    case "briar.mobile.v1.MessageBlock":
      if (typeof source.type === "string") adapted[source.type] = source;
      break;
    case "briar.mobile.v1.RichTextInline":
      if (typeof source.type === "string") adapted[source.type] = source;
      break;
    case "briar.mobile.v1.RichTextElement": {
      const kind = {
    ...
    case "briar.mobile.v1.ChannelProposal": {
...
const callChannelRoute = async (
  input: MobileConnectChannelInput,
  services: MobileConnectChannelServices,
  route: ChannelRoute,
  path: string,
...
const createMobileChannelService = (
  input: MobileConnectChannelInput,
  services: MobileConnectChannelServices = mobileConnectChannelServices,
...
export function registerMobileChannelService(
  router: ConnectRouter,
  input: MobileConnectChannelInput,
  services: MobileConnectChannelServices = mobileConnectChannelServices,
) {
  router.service(ChannelService, createMobileChannelService(input, services));
}

export { ChannelService, createMobileChannelService };
