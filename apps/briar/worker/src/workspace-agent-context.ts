...
export class WorkspaceAgentContextCursorError extends Error {
  constructor(message = "Invalid workspace context cursor") {
    super(message);
    this.name = "WorkspaceAgentContextCursorError";
  }
}

export class WorkspaceAgentContextPageTooLargeError extends Error {
  constructor() {
    super("One workspace context item exceeds the encoded page limit");
    this.name = "WorkspaceAgentContextPageTooLargeError";
  }
}
...
const decodeCursor = (
  ...
  if (encoded.length > 4_096 || !/^[A-Za-z0-9_-]+$/u.test(encoded)) {
    throw new WorkspaceAgentContextCursorError();
  }
  try {
    ...
    if (
      ...
      throw new WorkspaceAgentContextCursorError(
        "Workspace context cursor belongs to another claim or resource",
      );
    }
    return cursor;
  } catch (error) {
    if (error instanceof WorkspaceAgentContextCursorError) throw error;
    throw new WorkspaceAgentContextCursorError();
  }
};
...
const pageLimit = (limit: number | undefined) => {
  ...
  if (
    ...
    throw new RangeError(
      `Workspace context page size must be 1-${organizationAgentContextMaxPageSize}`,
    );
  }
  return value;
};
...
>(input: {
  ...
  while (true) {
    ...
    if (included <= 1) {
      throw new WorkspaceAgentContextPageTooLargeError();
    }
    included -= 1;
  }
};
...
const getContextSession = async (
  ...
  if (!metadata) {
    throw new Error(
      `Workspace context session disappeared while paging: ${input.sessionId}`,
    );
  }
    ...
    try {
      ...
      if (
        ...
        error.message.startsWith(
          "Workspace context session disappeared while paging:",
        )
      ) {
        return null;
      }
...
export async function lookupWorkspaceAgentContext(
  db: D1Database,
  archives: ArchiveBucket,
  input: ContextPageInput & {
