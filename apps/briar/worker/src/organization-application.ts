 ...
 * protobuf still call the same value an organization -- `briar_organizations`,
 * `organization_id`, `OrganizationService` -- so the repository symbols this
 * module imports keep their storage names, and the `organizationId` ->
 * `workspaceId` mapping happens once, at the Connect handler in
 * `app-connect-organization.ts`.
 *
...
export type WorkspaceApplicationServices = {
  ...
  readonly listWorkspaces: typeof listOrganizations;
