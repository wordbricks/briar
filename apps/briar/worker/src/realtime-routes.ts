...
import { subscribeToWorkspaceRealtime } from "./channel-realtime";
import {
...
export async function handleRealtimeRoute(
  ...
  const channelEventsMatch = pathname.match(
    /^\/workspaces\/([0-9a-f-]+)\/channel-events$/u,
  );
  if (channelEventsMatch && request.method === "GET") {
    const workspaceId = channelEventsMatch[1];
    ...
    const [channels, inbox] = await Promise.all([
      getChannelSyncCursor(db, workspaceId),
      getWorkspaceInboxSyncVersion(db, workspaceId),
    ]);
    return subscribeToWorkspaceRealtime(env, workspaceId, {
      channels,
      inbox,
    });
  }
  ...
  const channelActivityEventsMatch = pathname.match(
    /^\/workspaces\/([0-9a-f-]+)\/channels\/([0-9a-f-]+)\/agent-activity-events$/u,
  );
  if (channelActivityEventsMatch && request.method === "GET") {
    if (request.headers.get("Upgrade")?.toLowerCase() !== "websocket") {
      throw new HttpError(426, "WebSocket transport required");
    }
    const [workspaceId, channelId] = channelActivityEventsMatch.slice(1);
    const ticket = new URL(request.url).searchParams.get("ticket") ?? "";
