...
export async function claimNextChannelReplyWork(
  ...
  return releasingChannelReplyClaimOnFailure(
    { db, env, context, workspaceId: input.workspaceId, jobId: job.id,
      deviceId: principal.deviceId, workerId: binding.id, claimTokenHash },
    () => claimedChannelReplyPayload({
      db, env, job, principal, binding, runtime, claimToken, claimTokenHash,
      observedAt,
    }),
  );
}

type ClaimedChannelReplyJob = NonNullable<
  Awaited<ReturnType<typeof claimNextChannelAgentReply>>
>;

type ClaimedChannelReplyPayloadInput = {
  db: D1Database;
  env: Env;
  job: ClaimedChannelReplyJob;
  principal: AuthenticatedChannelWorkerProject["principal"];
  binding: NonNullable<AuthenticatedChannelWorkerProject["binding"]>;
  runtime: ReturnType<typeof executionWorkerRuntime>;
  claimToken: string;
...
/**
 * The claim payload, built from a job row this Worker already holds. Both the
 * claim itself and the pre-turn steer fold return exactly this, so the field
 * list has one definition and the folded turn cannot drift away from a claimed
 * one.
 */
async function claimedChannelReplyPayload(
  payloadInput: ClaimedChannelReplyPayloadInput,
) {
  const { db, env, job, principal, binding, runtime, claimToken,
    claimTokenHash, observedAt } = payloadInput;
...
/**
 * The claim mutation commits before the payload is built, so a build failure
 * would otherwise leave the job `running` behind a lease no Worker is
 * executing. Release the claim instead, so the job requeues immediately.
 */
async function releasingChannelReplyClaimOnFailure<T>(
  release: {
    db: D1Database;
    env: Env;
    context?: ExecutionContext;
  ...
  try {
    ...
    await failChannelReply(release.db, {
      jobId: release.jobId,
      deviceId: release.deviceId,
      workerId: release.workerId,
      claimTokenHash: release.claimTokenHash,
...
export type RefreshChannelReplyClaimInput = {
  input: {
    workspaceId: string;
    workerId: string;
    jobId: string;
...
/**
 * Folds a steer that landed after the claim into the running claim, without a
 * restart, and answers with the claim payload rebuilt against the folded input.
 *
 * Before this existed the only mid-turn steer check was the lease renewal, and
 * a plain direct message renewed every five minutes: a second message sent
 * seconds after the first was noticed only when the finished answer was
 * refused, after a whole provider turn, and cost a requeue and a second setup
 * on top. Null means nothing was pending and the Worker keeps the payload it
 * already holds.
 */
export async function refreshChannelReplyClaim(
  refreshInput: RefreshChannelReplyClaimInput,
) {
  const { input, db, env, context, authenticatedWorker } = refreshInput;
  const principal = authenticatedWorker.principal;
  if (principal.workspaceId !== input.workspaceId) {
    throw new HttpError(403, "Worker is not enabled for this workspace");
  }
  ...
  const held = await getClaimedChannelReply(db, claim);
  if (!held || held.organization_id !== input.workspaceId) {
    throw new HttpError(409, "Reply claim is no longer active");
  }
  if (held.steer_revision === held.applied_steer_revision) return null;
  const job = await applyChannelReplySteerInPlace(db, claim);
  ...
  return releasingChannelReplyClaimOnFailure(
    { db, env, context, workspaceId: input.workspaceId, jobId: job.id,
      deviceId: principal.deviceId, workerId: binding.id, claimTokenHash },
    () => claimedChannelReplyPayload({
      db, env, job, principal, binding, runtime, claimToken: input.claimToken,
      claimTokenHash, observedAt,
    }),
  );
}
