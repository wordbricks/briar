...
export type MobileConnectAgentInput = {
...
export type MobileConnectAgentServices = {
...
const mobileConnectAgentServices: MobileConnectAgentServices = {
  requireSession,
  getProject,
...
const jsonStruct = (value: unknown): JsonObject => value as JsonObject;
...
const toProjectAgentSession = (input: {
  ...
  dispatchEvents?: readonly unknown[];
  workers?: readonly unknown[];
}): ProjectAgentSession => ({
  $typeName: "briar.mobile.v1.ProjectAgentSession",
  id: input.id,
  ...
  followUps: input.payload.followUps.map((followUp) => ({
    $typeName: "briar.mobile.v1.ProjectAgentSessionFollowUp",
    id: followUp.id,
  ...
  issues: input.payload.issues.map((issue) => ({
    $typeName: "briar.mobile.v1.ProjectAgentSessionIssue",
    runId: issue.runId,
...
const rowToProjectAgentSession = (
  row: Awaited<ReturnType<typeof getProjectAgentSession>> extends infer R
    ? Exclude<R, null>
    : never,
  options: { archived?: boolean; detailLoaded?: boolean } = {},
) => {
  ...
  return toProjectAgentSession({
    ...
    detailLoaded: options.detailLoaded,
  });
};
...
/** Generated Connect implementation for the mobile AgentService. */
export const createMobileAgentService = (
  { request, auth, db, env, context }: MobileConnectAgentInput,
  services: MobileConnectAgentServices = mobileConnectAgentServices,
) => ({
...
export function registerMobileAgentService(
  router: ConnectRouter,
  input: MobileConnectAgentInput,
  services: MobileConnectAgentServices = mobileConnectAgentServices,
) {
  router.service(AgentService, createMobileAgentService(input, services));
}
