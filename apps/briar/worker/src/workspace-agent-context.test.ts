...
beforeAll(async () => {
  ...
  for (const [id, name, handle, ownerId] of [
    [workspaceId, "Context Org", "context-org", "owner-a"],
    [otherWorkspaceId, "Other Org", "other-org", "owner-b"],
  ]) {
    await db.prepare(
...
describe("Workspace Agent context lookup", () => {
  it("builds a revision manifest without embedding settings or retained payloads", async () => {
    ...
    expect(() => decodeWorkspaceAgentContextManifest(manifest))
      .not.toThrow();
  ...
  it("returns summaries first and batches only explicitly requested details", async () => {
    const summaries = await lookupWorkspaceAgentContext(db, archives, {
      workspaceId,
      workId,
      snapshotAt,
      requests: [
    ...
    const details = await lookupWorkspaceAgentContext(db, archives, {
      workspaceId,
      workId,
      snapshotAt,
      requests: [
  ...
  it("rejects malformed cursors and cursors from another lookup scope", async () => {
    const first = await lookupWorkspaceAgentContext(db, archives, {
      workspaceId,
      workId,
      snapshotAt,
      requests: [{
    ...
    const lookupWithCursor = (
      ...
      lookupWorkspaceAgentContext(db, archives, {
        workspaceId,
    ...
    await expect(lookupWithCursor({
      ...
      workId: otherWorkId,
    })).rejects.toBeInstanceOf(WorkspaceAgentContextCursorError);
    await expect(lookupWithCursor({
      cursor: page.nextCursor!,
      projectId: secondProjectId,
    })).rejects.toBeInstanceOf(WorkspaceAgentContextCursorError);
  ...
  it("fits large summary pages to the byte budget", async () => {
    ...
    const response = await lookupWorkspaceAgentContext(db, archives, {
      workspaceId,
      workId,
      snapshotAt,
      requests: [{
