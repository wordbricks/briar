...
describe("D1 migrations", () => {
  it("retires v1 archive metadata through cleanup before accepting v2", async () => {
    const miniflare = new Miniflare({
      modules: true,
      script: "export default { fetch() { return new Response('ok') } }",
      d1Databases: { DB: "briar-archive-v2-migration-test" },
    });
    try {
      const db = (await miniflare.getD1Database("DB")) as unknown as D1Database;
      ...
      const insertArchive = (id: string, objectKey: string, formatVersion: number) =>
        db.prepare(
          `insert into briar_log_archives (
             id, project_id, run_id, scope_id, archive_kind, object_key,
             format_version, status, row_count, byte_size, sha256,
  ...
  it("canonicalizes persisted channel issue proposal status", async () => {
    const miniflare = new Miniflare({
      modules: true,
      script: "export default { fetch() { return new Response('ok') } }",
      d1Databases: { DB: "briar-channel-proposal-status-migration-test" },
    });
    try {
      const db = (await miniflare.getD1Database("DB")) as unknown as D1Database;
      const now = "2026-08-31T00:00:00.000Z";
      ...
      await createPreDirectMessageMigrationChannel(db, {
        id: "proposal-channel",
        organizationId: "proposal-org",
        slug: "proposals",
        name: "Proposals",
        topic: null,
        visibility: "public",
        defaultProjectId: "proposal-project",
        createdByUserId: "proposal-owner",
        createdAt: now,
      });
