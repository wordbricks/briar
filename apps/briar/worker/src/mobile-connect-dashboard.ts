...
import {
  mobileAgentProvider,
  mobileChannelNotification,
  mobileConversationNotification,
  mobileDashboardRun,
...
export type MobileConnectDashboardInput = {
...
export const createMobileDashboardService = (
  { request, auth, db, archivesBucket }: MobileConnectDashboardInput,
) => ({
  getDashboard: async (rpcRequest: { readonly projectId: string }) =>
    withConnectErrors(async () => {
      ...
      return {
        ...
        settings: mobileProjectSettings(settingsJson(
          projectSettings,
          checkpointPolicyJson(checkpointPolicy),
        )),
        runs: runs.map((run) =>
          mobileDashboardRun(runJsonWithRelations(run, relations))
        ),
        workers: workers.map((worker) =>
          mobileDashboardWorker(workerJson(worker, observedAt))
        ),
        organizationProviders: organizationProviders.map(
          (provider) => mobileAgentProvider[provider],
        ),
        executionPolicy: mobileExecutionPolicy(executionPolicy),
        members: members.map((member) => mobileOrganizationMember(member)),
        conversationNotifications: conversationNotifications.map(
          (notification) => mobileConversationNotification(
  ...
  syncDashboard: async (rpcRequest: {
    ...
    return {
      ...
      project: metadata ? mobileProject(project) : undefined,
      settings: metadata
        ? mobileProjectSettings(settingsJson(
            metadata[0],
      ...
      executionPolicy: metadata
        ? mobileExecutionPolicy(metadata[2])
        : undefined,
      members: metadata
        ? metadata[3].map((member) => mobileOrganizationMember(member))
        : [],
  ...
  listRunEvents: async (rpcRequest: {
    ...
    return {
      events: events.map((event) =>
        mobileRunEvent(dashboardEventJson(event, actorNames))
      ),
    };
  }),
});

export function registerMobileDashboardService(
  router: ConnectRouter,
  input: MobileConnectDashboardInput,
) {
  router.service(DashboardService, createMobileDashboardService(input));
}
