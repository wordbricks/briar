...
import {
  type ClaimedTeamAgentTaskRow,
  type TeamAgentProvider,
  type TeamAgentTaskCompletionReceiptRow,
  type TeamAgentTaskJobRow,
} from "./team-agent-model";

export async function createTeamAgentTaskJob(
  db: D1Database,
  input: {
    id: string;
    projectId: string;
  ...
  return getTeamAgentTaskJob(db, input.projectId, input.id);
}

export async function getTeamAgentTaskJob(
  db: D1Database,
  projectId: string,
  jobId: string,
) {
...
export async function getTeamAgentTaskJobByRequest(
  db: D1Database,
  projectId: string,
  requestId: string,
) {
  ...
  if (claimed.skill_execution_proposal_id) {
    const approval = await db
      ...
      .first<{
        ...
        agent_computer_use_policy: ClaimedTeamAgentTaskRow["agent_computer_use_policy"];
...
export async function renewTeamAgentTaskLease(
  db: D1Database,
  projectId: string,
  jobId: string,
  input: {
...
export async function completeTeamAgentTaskWithReceipt(
  db: D1Database,
  projectId: string,
  jobId: string,
  input: {
  ...
  const receipt = receiptResult.results[0] as
    ...
    | undefined;
  const job = completionResult.results[0] as TeamAgentTaskJobRow | undefined;
  if (receipt && job) {
  ...
  const existing = await db
    ...
    .first<TeamAgentTaskCompletionReceiptRow>();
  if (
  ...
  return {
    job: await getTeamAgentTaskJob(db, projectId, jobId),
    receipt: existing,
    replayed: true,
  };
}

export async function completeTeamAgentTask(
  db: D1Database,
  projectId: string,
  jobId: string,
  input: {
  ...
  return (await completeTeamAgentTaskWithReceipt(
    db,
    projectId,
    jobId,
    input,
