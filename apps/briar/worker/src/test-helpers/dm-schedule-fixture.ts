...
export async function dmScheduleFixture(db: D1Database, now = new Date().toISOString(), provider: AgentProvider = "codex") {
  const workspaceId = crypto.randomUUID(), projectId = crypto.randomUUID();
  ...
  await db.batch([
    ...
    db.prepare(`insert into briar_execution_worker_devices (id, organization_id, owner_user_id, label,
      ...
      values (?, ?, ?, 'Schedule Device', ?, 'online', ?, ?, ?)`).bind(deviceId, workspaceId, ownerId, deviceId.replaceAll("-", "").repeat(2), now, now, now),
  ...
  await createWorkspaceAgent(db, { id: agentId, workspaceId, name: "Schedule Assistant", provider,
    model: null, responsibility: "Answer scheduled work in the original DM.", effort: null, createdAt: now });
  await createChannel(db, { id: channelId, workspaceId, kind: "dm", dmKey: `schedule:${channelId}`,
  ...
  await db.batch([
    ...
    db.prepare(`insert into briar_channel_reply_sessions (id, organization_id, channel_id, thread_root_message_id,
      ...
      values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).bind(sessionId, workspaceId, channelId, sourceId, agentId, provider,
    ...
    db.prepare(`insert into briar_channel_agent_reply_jobs (id, organization_id, channel_id, agent_id, session_id,
      ...
      values (?, ?, ?, ?, ?, ?, ?, ?, ?, 'running', 'new', ?, ?, ?, ?, ?, ?, ?)`)
      .bind(jobId, workspaceId, channelId, agentId, sessionId, sourceId, sourceId, crypto.randomUUID(), provider,
        deviceId, workerId, claimTokenHash, now, new Date(Date.parse(now) + 86400_000).toISOString(), now, now),
  ]);
  const identity = { jobId, workspaceId, channelId, workerId, deviceId, claimTokenHash, observedAt: now };
