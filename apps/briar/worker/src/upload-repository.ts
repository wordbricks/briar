...
export type UploadPurpose = "issue_reply" | "channel_reply" | "run_evidence";
...
const sameNullable = (left: string | null, right: string | null) =>
  left === right;
...
export async function prepareUploadRows(
  ...
  const prepared = input.files.map((file, position) => {
    ...
    return {
      ...
      uploadId,
      objectKey:
        `uploads/${input.purpose}/${input.organizationId}/${input.requestId}/${uploadId}`,
    };
  });
  ...
  const batch = insertedBatch ?? await db.prepare(
    `select * from briar_upload_batches where request_id = ?`,
  ).bind(input.requestId).first<UploadBatchRow>();
  ...
  ).bind(input.requestId).all<UploadRow>();
...
export async function getScopedUpload(db: D1Database, uploadId: string) {
  ...
  ).bind(uploadId).first<ScopedUploadRow>();
}
...
export async function markUploadStored(
  ...
  return db.prepare(
    `update briar_uploads as upload
     ...
     returning *`,
  ).bind(observedAt, uploadId, observedAt).first<UploadRow>();
}

const scopeSql = `batch.purpose = ? and batch.organization_id = ?
  and batch.project_id is ? and batch.work_id is ? and batch.run_id is ?
  and batch.worker_id is ? and batch.device_id is ?
...
export async function resolveAvailableUploads(
  ...
  return ordered.some((upload) => upload === undefined)
    ? null
    : ordered as ScopedUploadRow[];
}
...
export function consumeUploadStatements(
  ...
  input: UploadScope & {
    ...
    consumerKind: "reply_completion" | "run_evidence";
    consumerId: string;
    consumedAt: string;
  },
) {
  return input.uploadIds.map((uploadId) => db.prepare(
    `update briar_uploads as upload
...
export async function enqueueExpiredUploadCleanup(
  db: D1Database,
  observedAt: string,
  limit = 100,
) {
...
export async function processUploadCleanupQueue(
  ...
  const due = await db.prepare(
    `select * from briar_upload_cleanup_queue
     ...
     limit ?`,
  ).bind(observedAt, limit).all<UploadCleanupRow>();
  let deleted = 0;
  let failed = 0;
