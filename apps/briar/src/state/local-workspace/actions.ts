...
import { demoWorkspace, emptyDashboard } from "../demo-fixtures";
    ...
    async addProject(input: { name: string }) {
      if (modes().demoMode) {
        const workspace =
          registry
            ...
            .find(
              (candidate) =>
                candidate.id === registry.get(activeWorkspaceIdAtom),
            ) ?? demoWorkspace;
      ...
      try {
        const result = await remote.createTeam(token, {
          ...input,
          workspaceId: registry.get(activeWorkspaceIdAtom) ?? undefined,
        });
        const nextWorkspaces = await remote
        ...
        Atom.batch(() => {
          ...
          registry.set(activeWorkspaceIdAtom, result.project.workspaceId);
    ...
    async removeProject(teamId: string): Promise<void> {
      ...
      return mutate(deleteTeamAction, teamId, async () => {
        ...
        Atom.batch(() => {
          ...
          if (nextActiveTeam?.workspaceId) {
            registry.set(
              activeWorkspaceIdAtom,
              nextActiveTeam.workspaceId,
            );
          }
