...
import { requestedWorkspaceAgentIdAtom } from "../state/dialogs/atoms";
...
export function WorkspaceAgentsSettings({
  ...
  const requestedAgentId = useAtomValue(requestedWorkspaceAgentIdAtom);
  const setRequestedAgentId = useAtomSet(requestedWorkspaceAgentIdAtom);

  useEffect(() => {
    let cancelled = false;
    setIsLoading(true);
    setError(null);
    void listWorkspaceAgents(token, workspaceId)
      .then((result) => {
        if (cancelled) return;
  ...
  const createAgent = async (input: {
    ...
    const result = await createWorkspaceAgent(token, workspaceId, input);
    ...
    <>
      <div className="mb-7 flex items-start justify-between gap-4">
        <SettingsPageHeader
          ...
          description={t("workspace.agentsDescription", {
            name: workspaceName,
          })}
          title={t("workspace.agentsTitle")}
        />
        {canManage ? (
          <Button
      ...
      {isLoading ? (
        ...
        >
          ...
          <Typography variant="bodySm">{t("workspace.agentsLoading")}</Typography>
        </div>
      ) : agents.length === 0 ? (
          ...
          <div className="grid max-w-md justify-items-center gap-3">
            ...
            <div>
              <Typography as="h2" variant="bodyLg">
                {t("workspace.agentsEmptyTitle")}
              </Typography>
              <Typography className="mt-1" tone="muted" variant="bodySm">
                {t("workspace.agentsEmptyDescription")}
              </Typography>
            </div>
            {canManage ? (
        ...
        <section
          aria-label={t("workspace.agentsList")}
            ...
            >
              ...
              {canManage ? (
                <div className="flex self-start">
                  ...
                  <Button
                    aria-label={t("workspace.agentsDelete", {
                      name: agent.name,
                    })}
              ...
              <AgentConversationsSection
                ...
                onOpenConversation={(channelId) =>
                  navigateToChannel(channelId, "dms", workspaceId)}
      ...
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              {t("workspace.agentsDeleteTitle", {
                name: deletingAgent?.name ?? "",
              })}
            </DialogTitle>
            <DialogDescription>
              {t("workspace.agentsDeleteDescription")}
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button
            ...
            >
              ...
              {t(
                isDeleting
                  ? "workspace.agentsDeleting"
                  : "workspace.agentsDeleteAction",
              )}
            </Button>
          </DialogFooter>
        </DialogContent>
...
function WorkspaceAgentCreateDialog({
  isOpen,
  onClose,
  onCreate,
}: {
  ...
  return (
    ...
    >
      <DialogContent className="sm:max-w-xl">
        <DialogHeader>
          <DialogTitle>{t("workspace.agentsCreateTitle")}</DialogTitle>
          <DialogDescription>
            {t("workspace.agentsCreateDescription")}
          </DialogDescription>
        </DialogHeader>
        ...
        <form
          ...
          id="workspace-agent-create-form"
          onSubmit={(event) => {
        ...
        >
          <div className="grid gap-2">
            <Label htmlFor="workspace-agent-name">
            ...
            <Input
              ...
              placeholder={t("workspace.agentsNamePlaceholder")}
              required
              value={name}
            />
          </div>
          ...
          <div className="grid gap-2">
            <Label htmlFor="workspace-agent-description">
              {t("agents.agentDescription")} · {t("common.optional")}
            </Label>
            <Textarea
              id="workspace-agent-description"
          ...
          <div className="grid gap-2">
            <Label htmlFor="workspace-agent-responsibility">
              {t("agents.responsibility")} · {t("common.required")}
            </Label>
            <Textarea
              id="workspace-agent-responsibility"
            ...
            />
            <Typography tone="muted" variant="caption">
              {t("workspace.agentsResponsibilityHint")}
            </Typography>
          </div>
        </form>
        ...
        <DialogFooter>
          ...
          <Button
            disabled={!canSubmit}
            form="workspace-agent-create-form"
            type="submit"
          >
            {isSubmitting ? (
              <Spinner className="size-[16px]" />
            ...
            {t(
              isSubmitting
                ? "workspace.agentsCreating"
                : "workspace.agentsCreate",
            )}
          </Button>
        </DialogFooter>
      </DialogContent>
