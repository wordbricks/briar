...
export type TeamScheduleServices = {
...
const defaultTeamScheduleServices: TeamScheduleServices = {
  createProjectAgentSchedule,
  deleteProjectAgentSchedule,
  loadProjectAgentSchedules,
  loadProjectAgentScheduleRuns,
...
export function TeamSchedule({
  ...
  now: providedNow,
  project,
  services = defaultTeamScheduleServices,
  token,
}: {
  isSidebarOpen: boolean;
  now?: Date;
  project: Project;
  services?: TeamScheduleServices;
  token: string | null;
}) {
  ...
  useEffect(() => {
    ...
    const load = token
      ...
      : Promise.resolve([
          demoTeamAgents(project.id, locale),
...
export function TeamAgentScheduleDetails({
  occurrence,
  onClose,
  onDelete,
  onEdit,
...
export function DeleteTeamAgentScheduleDialog({
  isDeleting,
  onClose,
  onDelete,
  schedule,
...
export function CreateTeamAgentScheduleDialog({
  agents,
  isSubmitting,
  onClose,
  onCreate,
  ...
  const [recurrence, setRecurrence] = useState<TeamAgentScheduleRecurrence>(
  ...
  const [intervalUnit, setIntervalUnit] =
    useState<TeamAgentScheduleIntervalUnit>(
  ...
  const [notificationLevel, setNotificationLevel] =
    useState<TeamAgentScheduleNotificationLevel>(
  ...
  const normalizedIntervalValue =
    normalizeTeamAgentScheduleInterval(intervalValue);
  ...
  const canSubmit =
    ...
    isValidTeamAgentScheduleTime(timeOfDay) &&
    hasValidCustomDays;

  const changeRecurrence = (value: string) => {
    const next = value as TeamAgentScheduleRecurrence;
    setRecurrence(next);
    if (next === "interval") {
  ...
  const toggleDay = (day: number) => {
    setDaysOfWeek((current) =>
      current.includes(day)
        ...
        : normalizeTeamAgentScheduleDays([...current, day]),
    );
  };
      ...
      <form
        ...
        onSubmit={(event) => {
          ...
          const input = {
            ...
            recurrence,
            timeOfDay,
            dayOfWeek: normalizeTeamAgentScheduleDay(recurrence, dayOfWeek),
            ...
            {recurrence === "custom" && (
              <div>
                ...
                <NativeSelect
                  ...
                  onValueChange={(value) =>
                    setIntervalUnit(value as TeamAgentScheduleIntervalUnit)
                  }
                  options={[
                    {
                ...
                <div className="grid min-w-0 grid-cols-[82px_minmax(120px,1fr)] items-center gap-2 max-[760px]:grid-cols-[68px_minmax(0,1fr)] [&>input]:h-9 [&>input]:text-right [&>strong]:pr-2 [&>strong]:text-right [&>strong]:text-micro [&>strong]:font-medium [&>strong]:text-muted-foreground">
                  ...
                  {recurrence === "interval" ? (
                    <NativeSelect
                      ...
                      onValueChange={(value) =>
                        setIntervalUnit(
                          value as TeamAgentScheduleIntervalUnit,
                        )
                      }
                      options={[
                        {
              ...
              <NativeSelect
                ...
                onValueChange={(value) =>
                  setNotificationLevel(
                    value as TeamAgentScheduleNotificationLevel,
                  )
                }
                options={[
                  {
