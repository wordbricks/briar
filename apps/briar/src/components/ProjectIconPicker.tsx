...
import { projectIconColors, projectIcons } from "../lib/project-icon-library";
...
export function ProjectIconPicker({
  ...
  const visibleIcons = useMemo(() => {
    ...
    if (!normalized) return projectIcons;
      ...
      >
        ...
        <div className="grid max-h-72 grid-cols-8 gap-1 overflow-y-auto sm:grid-cols-10">
          {visibleIcons.map((icon) => {
            const Icon = projectIconComponent(icon.name);
