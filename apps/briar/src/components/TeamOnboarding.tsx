...
export function TeamOnboarding({
  ...
  const [workflowProgress, setWorkflowProgress] =
    useState<TeamLlmProgress | null>(null);
  ...
  const selectRepository = async () => {
    ...
    try {
      ...
      if (!connection) setName(repositoryTeamName(readiness.repositoryPath));
      try {
  ...
  const continueFromRepository = async (event: React.FormEvent) => {
    ...
    setConnectionPreflight(null);
    try {
      const preflight = await preflightThenCreateTeam(
