...
// matches Orca’s Claude/Codex branding. Grok and OpenRouter use the user-provided
// artwork bundled with Briar. OpenCode keeps Orca’s light-theme SVG and swaps to
// the user-provided dark-theme artwork. Cursor and Antigravity use the provider
// artwork already bundled with Briar.
...
function providerArtworkClassName(
  sourceTone: "dark" | "white",
  className?: string,
) {
  return ["provider-artwork", `provider-artwork-${sourceTone}`, className]
    .filter(Boolean)
    .join(" ");
}

export function CodexIcon({ size = 14, className, style }: AgentIconProps) {
  return (
    <svg
      aria-hidden
      className={["provider-artwork", "provider-artwork-codex", className]
        .filter(Boolean)
        .join(" ")}
      fill="currentColor"
      fillRule="evenodd"
...
export function ClaudeIcon({ size = 14, className, style }: AgentIconProps) {
  return (
    <svg
      aria-hidden
      className={className}
...
export function GrokIcon({ size = 14, className, style }: AgentIconProps) {
  return (
    <img
      alt=""
      aria-hidden
      className={providerArtworkClassName("white", className)}
      height={size}
      src={grokIconUrl}
...
export function OpenCodeIcon({ size = 14, className, style }: AgentIconProps) {
  return (
    <span
      aria-hidden="true"
      className={["provider-artwork-theme-switcher", className]
        .filter(Boolean)
        .join(" ")}
      style={{ display: "block", width: size, height: size, ...style }}
    >
      <svg
        aria-hidden
        className="provider-artwork-light"
        fill="none"
        height={size}
        viewBox="0 0 32 40"
        width={size}
      ...
      <img
        alt=""
        aria-hidden
        className={providerArtworkClassName("dark")}
        height={size}
        src={opencodeIconUrl}
...
export function AgentProviderIcon({
  ...
  provider: AgentProviderIconId;
  size?: number;
  className?: string;
  style?: CSSProperties;
}) {
  if (provider === "claude") {
    return <ClaudeIcon className={className} size={size} style={style} />;
  }
  if (provider === "grok") {
