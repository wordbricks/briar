...
describe("ChannelMessageText", () => {
  ...
  it("renders error and warning webhook dumps as alert cards and collapses stack traces", async () => {
    const webhookMessage: ChannelMessage = {
      ...message,
      author: { type: "webhook", id: "webhook-2", name: "Sentry" },
    ...
    const card = container.querySelector(".channel-alert-card");
    expect(card?.getAttribute("data-alert-tone")).toBe("error");
    expect(container.querySelector(".channel-alert-preview")).not.toBeNull();
    const toggle = container.querySelector<HTMLButtonElement>(
      ".channel-alert-toggle",
    );
    expect(toggle).not.toBeNull();
    await act(async () => toggle!.click());
    expect(container.textContent).toContain("at start (boot.ts:1:1)");
  });

  it("pretty-prints JSON error payloads in an error card", async () => {
    await act(async () => {
      root.render(
        <I18nProvider>
          <ChannelMessageText
            ...
            message={{
              ...message,
              author: { type: "webhook", id: "webhook-3", name: "PagerDuty" },
              body: '{"level":"error","message":"timeout","service":"api"}',
            }}
          />
        </I18nProvider>,
      );
    ...
    expect(
      container.querySelector(".channel-alert-card")?.getAttribute("data-alert-tone"),
    ).toBe("error");
    const toggle = container.querySelector<HTMLButtonElement>(
      ".channel-alert-toggle",
    );
