...
import { TeamAgentSettings } from "./TeamAgentSettings";
import { TeamAgentDesignatedWorkerSelect } from "./TeamAgentDesignatedWorkerSelect";
import {
  TeamAgentTaskDialog,
...
export function TeamAgents({
  ...
  onStartTaskSession: (
    agent: ProjectAgent,
    session: TeamAgentTaskSessionStart,
  ) => void;
  project: Project;
  ...
  useEffect(() => {
    ...
    const load = token
      ...
      : Promise.resolve(demoTeamAgents(project.id, locale));
    void load
      .then((nextAgents) => {
...
export function TeamAgentDialog({
  ...
  const [calendarColor, setCalendarColor] = useState(
    agent?.calendarColor ?? defaultTeamAgentCalendarColor,
  );
  ...
  const startBlankAgent = () => {
    ...
    setDescription("");
    setResponsibility("");
    setCalendarColor(defaultTeamAgentCalendarColor);
    setCreationStep("form");
    setSubmitError(null);
  };
...
function localProjectAgent(
  ...
  return {
    ...
    description: input.description?.trim() ?? "",
    responsibility: input.responsibility,
    skill: teamAgentSkill({ name, responsibility: input.responsibility }),
...
function localProjectAgentSkills(
  agentId: string,
  inputs: readonly TeamAgentSkillInput[],
  updatedAt: string,
