...
import { WorkspaceWorkersSettings } from "./WorkspaceWorkersSettings";
import { WorkspaceIntegrationsSettings } from "./WorkspaceIntegrationsSettings";
import { WorkspaceAgentsSettings } from "./WorkspaceAgentsSettings";
import { SelectMenu } from "./SelectMenu";
import type { WorkspaceSettingsSection } from "../lib/app-navigation";

export type { WorkspaceSettingsSection } from "../lib/app-navigation";

type RoleFilter = "all" | WorkspaceMember["role"];
...
export function WorkspaceSettings({
  ...
  onRename: (workspaceId: string, name: string) => Promise<Workspace>;
  ...
  const [activeSection, setActiveSection] =
    useState<WorkspaceSettingsSection>(initialSection ?? "general");
  ...
  const [role, setRole] = useState<WorkspaceAssignableRole>("viewer");
  ...
  const canSaveWorkspaceName =
    canManage &&
    normalizedWorkspaceName.length > 0 &&
    normalizedWorkspaceName.length <= 100 &&
  ...
  useEffect(() => {
    ...
    setLoading(true);
    void Promise.all([
      loadWorkspaceMembers(token, workspaceId),
      canManage
        ? loadWorkspaceInvitations(token, workspaceId)
        : Promise.resolve([]),
    ])
  ...
  const roleLabel = (memberRole: WorkspaceMember["role"]) =>
    t(`workspace.role.${memberRole}` as
  ...
  const exportMembers = () => {
    ...
    anchor.href = url;
    anchor.download = `${workspace.name}-members.csv`;
    anchor.click();
    ...
    <SettingsShell className="bg-background">
      ...
      >
        ...
        <SettingsIdentity
          ...
          subtitle={t("workspace.settingsLabel")}
          title={workspace.name}
        />

        <SettingsNav>
          <SettingsNavGroup label={t("workspace.workspaceSection")}>
            <SettingsNavItem
            ...
            {activeSection === "general" ? (
                ...
                <section className="w-full max-w-[820px]">
                          ...
                          <div className="flex flex-wrap gap-2">
                            ...
                            >
                              <label>
                                ...
                                {t(
                                  workspace.logo
                                    ? "workspace.replaceLogo"
                                    : "workspace.uploadLogo",
                                )}
                                <input
                                  accept={organizationLogoAccept}
                                  aria-label={t("workspace.uploadLogo")}
                                  className="sr-only"
                                  ...
                                  onChange={(event) => {
                                    ...
                                    void organizationLogoFromFile(file)
                                      .then((logo) =>
                                        onLogoChange(workspaceId, logo),
                                      )
                                      .then(() => setLogoSaved(true))
                                      .catch(() =>
                                        setLogoError(
                                          t("workspace.logoUploadFailed"),
                                        ),
                                      )
                            ...
                            {workspace.logo ? (
                              <Button
                                ...
                                onClick={() => {
                                  ...
                                  setLogoError(null);
                                  setLogoSaved(false);
                                  void onLogoChange(workspaceId, null)
                                    .then(() => setLogoSaved(true))
                                    .catch(() =>
                                      setLogoError(
                                        t("workspace.logoUploadFailed"),
                              ...
                              >
                                ...
                                {t("workspace.removeLogo")}
                              </Button>
                            ) : null}
                          </div>
                        ) : (
                          <Typography tone="muted" variant="caption">
                            {t("workspace.logoPermission")}
                          </Typography>
                        )}
                  ...
                  <form
                    ...
                    onSubmit={(event) => {
                      ...
                      setRenameError(null);
                      setRenameSaved(false);
                      void onRename(workspaceId, normalizedWorkspaceName)
                  ...
                  >
                    <div>
                      ...
                      <Typography className="mt-1.5" tone="muted" variant="caption">
                        {t("workspace.workspaceNameDescription")}
                      </Typography>
                    </div>
                    <div className="grid grid-cols-[minmax(0,1fr)_auto] gap-2">
                      <Input
                        ...
                        value={workspaceName}
                      />
                      <Button disabled={!canSaveWorkspaceName} type="submit">
              ...
              <>
                <SettingsPageHeader
                  ...
                  description={t("workspace.membersDescription", {
                    name: workspace.name,
                  })}
                ...
                <div className="mb-4 flex flex-wrap items-center gap-2">
                  ...
                  <SelectMenu
                    ...
                    label={t("workspace.roleFilter")}
                    onValueChange={(value) => setRoleFilter(value as RoleFilter)}
                    options={[
                      { label: t("workspace.filterAll"), value: "all" },
                      { label: t("workspace.role.owner"), value: "owner" },
                      { label: t("workspace.role.co-owner"), value: "co-owner" },
                      { label: t("workspace.role.developer"), value: "developer" },
                      { label: t("workspace.role.editor"), value: "editor" },
                      { label: t("workspace.role.viewer"), value: "viewer" },
                    ]}
                    size="small"
                    value={roleFilter}
                  />
                ...
                <section
                  aria-label={t("workspace.memberList")}
                ...
                >
                  <div className="grid grid-cols-[minmax(0,1.2fr)_minmax(0,1.3fr)_110px_140px_90px_40px] gap-3 border-b border-border px-4 py-2.5 text-micro font-medium tracking-wide text-muted-foreground uppercase">
                    ...
                    <span>{t("workspace.joined")}</span>
                    <span className="sr-only">{t("workspace.actions")}</span>
                  </div>
                          ...
                          <SelectMenu
                            ...
                            onValueChange={(value) => {
                              ...
                              void updateWorkspaceMemberRole(
                                ...
                                workspaceId,
                                member.userId,
                                value as WorkspaceAssignableRole,
                              )
                ...
                {canManage ? (
                  <section
                    aria-label={t("workspace.pendingInvites")}
                    ...
                    <div className="flex items-center justify-between border-b border-border bg-muted/60 px-4 py-3">
                      <div>
                        ...
                        <Typography
                          ...
                          variant="caption"
                        >
                          {t("workspace.pendingInvitesDescription")}
                        </Typography>
                      </div>
                    ...
                    {invitations.length === 0 ? (
                      ...
                      invitations.map((invitation) => (
                        ...
                        >
                          ...
                          <Button
                            aria-label={t("workspace.revokeInvite", {
      ...
      >
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>
              {t("workspace.projectAccessTitle", {
                name: projectAccessMember?.name ?? "",
              })}
            </DialogTitle>
            <DialogDescription>
              {t("workspace.projectAccessDescription")}
            </DialogDescription>
          </DialogHeader>
          ...
          <DialogFooter className="mt-6">
            ...
            <Button
              ...
              onClick={() => {
                ...
                setError(null);
                void updateWorkspaceMemberProjects(
                  token,
                  workspaceId,
        ...
        <DialogContent className="sm:max-w-md">
          <form
            onSubmit={(event) => {
              ...
              setSaving(true);
              setError(null);
              void createWorkspaceInvitation(token, workspaceId, {
                email,
                initialProjectId,
                role,
              })
          ...
          >
            ...
            {inviteUrl ? (
              <div className="mt-5 grid gap-3">
                <div className="rounded-xl border border-success/30 bg-success/10 p-4">
                  ...
                  >
                    ...
                    {t("workspace.inviteLinkReady")}
                  </Typography>
                  <Typography className="mt-1" tone="muted" variant="caption">
                    {t("workspace.inviteLinkDescription")}
                  </Typography>
                </div>
              ...
              <div className="mt-4 grid gap-4">
                ...
                <div className="grid gap-2">
                  <Label>{t("workspace.inviteProject")}</Label>
                  <SelectMenu
                    disabled={organizationProjects.length === 0}
                    label={t("workspace.inviteProject")}
                    onValueChange={setInitialProjectId}
                    options={organizationProjects.map((project) => ({
                      label: project.name,
                      value: project.id,
                    }))}
                    placeholder={t("workspace.inviteProject")}
                    value={initialProjectId}
                  />
                  <Typography tone="muted" variant="caption">
                    {t("workspace.inviteProjectDescription")}
                  </Typography>
                </div>
                <div className="grid gap-2">
                  ...
                  <SelectMenu
                    ...
                    onValueChange={(value) =>
                      setRole(value as WorkspaceAssignableRole)
                    }
                    options={assignableRoleOptions}
                    value={role}
                  />
                  <Typography tone="muted" variant="caption">
                    {t(`workspace.roleDescription.${role}` as
                      | "workspace.roleDescription.co-owner"
                      | "workspace.roleDescription.developer"
                      | "workspace.roleDescription.editor"
                      | "workspace.roleDescription.viewer")}
                  </Typography>
                </div>
                {error ? (
