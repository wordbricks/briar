...
import type { TeamMergeActivity as Activity } from "../lib/team-merge-activity";
...
describe("TeamMergeActivity", () => {
  it("renders PR links and supports keyboard inspection of the trend", async () => {
    ...
    await renderReactTestRoot(root, <I18nProvider><TeamMergeActivity projectId="one" repository="briar/app" onLoad={async () => activity} /></I18nProvider>);
  ...
  it("does not request data without a repository and allows retry after a failure", async () => {
    ...
    const { root, container, cleanup } = createReactTestRoot();
    const render = (repository: string | null) => renderReactTestRoot(root, <I18nProvider><TeamMergeActivity projectId="one" repository={repository} onLoad={onLoad} /></I18nProvider>);
    await render(null);
  ...
  it("aborts outdated requests and never shows a previous project's response", async () => {
    ...
    const { root, container, cleanup } = createReactTestRoot();
    const render = (id: string, repository: string) => renderReactTestRoot(root, <I18nProvider><TeamMergeActivity projectId={id} repository={repository} onLoad={onLoad} /></I18nProvider>);
