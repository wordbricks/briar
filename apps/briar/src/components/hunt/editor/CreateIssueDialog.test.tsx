...
import { defaultAgentProviderModelCatalog, loadAgentProviderModels } from "@/lib/project-llm";
import { CreateIssueDialog } from "@/components/HuntDashboard";
...
vi.mock("@/lib/project-llm", async importOriginal => {
  ...
  return {
    ...original,
    loadAgentProviderModels: vi.fn(async () => original.defaultAgentProviderModelCatalog)
  };
});
const projects = [{
  id: "project-1",
...
Object.assign(globalThis, { IS_REACT_ACT_ENVIRONMENT: true });
  ...
  it("inserts a pasted image at the description caret and submits its reference", async () => {
    const onCreate = vi.fn<(projectId: string, input: CreateIssueInput) => Promise<void>>(async () => undefined);
    const root = createRoot(container);
    await act(async () => {
      root.render(<CreateIssueDialog {...projectProps} isSubmitting={false} members={[{
        userId: "user-1",
        name: "Kim",
    ...
    expect(Array.from(container.querySelectorAll<HTMLTextAreaElement>("textarea")).map(segment => segment.value).join("")).toBe("before\n\n\n\n after");
    expect(container.querySelector<HTMLImageElement>(".issue-inline-attachment img")?.src).toBe("blob:clipboard-preview");
    expect(Array.from(container.querySelectorAll<HTMLTextAreaElement>("textarea")).map(segment => segment.value).join("")).not.toContain("briar-attachment://");
    await act(async () => {
    ...
    expect(submitted.description).toContain(`briar-attachment://${submitted.attachmentReferences?.[0]}`);
    await act(async () => root.unmount());
  });
  it("adds a dropped image and shows drag feedback", async () => {
    const root = createRoot(container);
    await act(async () => {
      root.render(<CreateIssueDialog {...projectProps} isSubmitting={false} onClose={() => undefined} onCreate={async () => undefined} />);
    });
    ...
    expect(container.querySelector<HTMLImageElement>(".issue-inline-attachment img")?.alt).toBe("dropped.png");
    expect(container.querySelector<HTMLImageElement>(".issue-inline-attachment img")?.src).toBe("blob:clipboard-preview");
    await act(async () => root.unmount());
  });
  it("submits with Command+Enter when the title is present", async () => {
    ...
    const root = createRoot(container);
    await act(async () => {
      root.render(<CreateIssueDialog {...projectProps} isSubmitting={false} onClose={() => undefined} onCreate={onCreate} />);
    });
    ...
    expect(onCreate).toHaveBeenCalledWith("project-1", {
      assigneeUserId: null,
      attachments: [],
      description: null,
      fullAuto: false,
  ...
  it("moves focus to the description with Enter from the title input", async () => {
    ...
    expect(document.activeElement).toBe(container.querySelector(".issue-description-input"));
  ...
  it("does not submit Enter while the title is being composed", async () => {
    ...
    await act(async () => {
      container.querySelector<HTMLInputElement>(".issue-title-input")?.dispatchEvent(enterEvent);
    });
  ...
  it("creates an issue in backlog when that status is selected", async () => {
    ...
    await act(async () => {
      ...
      container.querySelector<HTMLButtonElement>(".issue-status-select .select-menu-trigger")?.click();
    });
    await act(async () => {
      document.querySelector<HTMLButtonElement>('[role="option"][data-value="backlog"]')?.click();
    });
    await act(async () => {
      container.querySelector<HTMLButtonElement>(".issue-assignee-select .select-menu-trigger")?.click();
    });
    await act(async () => {
      document.querySelector<HTMLButtonElement>('[role="option"][data-value="user-1"]')?.click();
    });
    await act(async () => {
      container.querySelector<HTMLFormElement>("form")?.requestSubmit();
    });
  ...
  it("selects a preferred provider and model when creating an issue", async () => {
    ...
    await act(async () => {
      document.querySelector<HTMLButtonElement>('[role="option"][data-value="xhigh"]')?.click();
    });
    await act(async () => {
      container.querySelector<HTMLFormElement>("form")?.requestSubmit();
    });
  ...
  it("defaults to the active project and can create in another organization project", async () => {
    ...
    const projectTrigger = container.querySelector<HTMLButtonElement>(".issue-project-context .select-menu-trigger");
    expect(projectTrigger?.textContent).toContain("GG");
    await act(async () => projectTrigger?.click());
    await act(async () => {
      document.querySelector<HTMLButtonElement>('[role="option"][data-value="project-2"]')?.click();
    });
    expect(projectTrigger?.textContent).toContain("Mobile");
    const titleInput = container.querySelector<HTMLInputElement>(".issue-title-input");
    await act(async () => {
      Object.getOwnPropertyDescriptor(HTMLInputElement.prototype, "value")?.set?.call(titleInput, "Cross-project issue");
      titleInput?.dispatchEvent(new Event("input", {
        bubbles: true
      }));
      container.querySelector<HTMLFormElement>("form")?.requestSubmit();
    });
    expect(onCreate).toHaveBeenCalledWith("project-2", expect.objectContaining({
  ...
  it("restores a draft after the backdrop closes the dialog", async () => {
    ...
    const root = createRoot(container);
    await act(async () => {
      root.render(<CreateIssueDialog {...projectProps} isSubmitting={false} onClose={onClose} onCreate={async () => undefined} />);
    });
    const titleInput = container.querySelector<HTMLInputElement>(".issue-title-input");
    const descriptionInput = container.querySelector<HTMLTextAreaElement>(".issue-description-input");
    await act(async () => {
      Object.getOwnPropertyDescriptor(HTMLInputElement.prototype, "value")?.set?.call(titleInput, "Accidentally closed issue");
      titleInput?.dispatchEvent(new Event("input", {
        bubbles: true
      }));
      Object.getOwnPropertyDescriptor(HTMLTextAreaElement.prototype, "value")?.set?.call(descriptionInput, "Keep this description");
      descriptionInput?.dispatchEvent(new Event("input", {
        bubbles: true
      }));
    });
    await act(async () => {
      container.querySelector<HTMLButtonElement>(".issue-project-context .select-menu-trigger")?.click();
    });
    await act(async () => {
      document.querySelector<HTMLButtonElement>('[role="option"][data-value="project-2"]')?.click();
      container.querySelector<HTMLButtonElement>(".issue-status-select .select-menu-trigger")?.click();
    ...
    await act(async () => {
      document.querySelector<HTMLButtonElement>('[role="option"][data-value="backlog"]')?.click();
      container.querySelector<HTMLButtonElement>(".issue-priority-select .select-menu-trigger")?.click();
    });
    await act(async () => {
      document.querySelector<HTMLButtonElement>('[role="option"][data-value="4"]')?.click();
    });
    ...
    expect(JSON.parse(window.localStorage.getItem(createIssueDraftStorageKey)!)).toEqual({
      assigneeUserId: null,
    ...
    await act(async () => {
      restoredRoot.render(<CreateIssueDialog {...projectProps} isSubmitting={false} onClose={() => undefined} onCreate={restoredOnCreate} />);
    });
    expect(container.querySelector<HTMLInputElement>(".issue-title-input")?.value).toBe("Accidentally closed issue");
    expect(container.querySelector<HTMLTextAreaElement>(".issue-description-input")?.value).toBe("Keep this description");
    expect(container.querySelector(".issue-project-context .select-menu-trigger")?.textContent).toContain("Mobile");
    await act(async () => {
      container.querySelector<HTMLFormElement>("form")?.requestSubmit();
    });
    expect(restoredOnCreate).toHaveBeenCalledWith("project-2", expect.objectContaining({
      description: "Keep this description",
      priority: 4,
      status: "backlog",
      title: "Accidentally closed issue"
  ...
  it("clears the saved draft after issue creation succeeds", async () => {
    window.localStorage.setItem(createIssueDraftStorageKey, JSON.stringify({
  ...
  it("keeps the saved draft when issue creation fails", async () => {
    const root = createRoot(container);
    await act(async () => {
      root.render(<CreateIssueDialog {...projectProps} isSubmitting={false} onClose={() => undefined} onCreate={async () => {
        throw new Error("Creation failed");
      }} />);
    });
    const titleInput = container.querySelector<HTMLInputElement>(".issue-title-input");
    await act(async () => {
      Object.getOwnPropertyDescriptor(HTMLInputElement.prototype, "value")?.set?.call(titleInput, "Retry this issue");
      titleInput?.dispatchEvent(new Event("input", {
        bubbles: true
      }));
      container.querySelector<HTMLFormElement>("form")?.requestSubmit();
    });
    expect(JSON.parse(window.localStorage.getItem(createIssueDraftStorageKey)!).title).toBe("Retry this issue");
    expect(container.querySelector(".issue-form-error")?.textContent).toContain("Creation failed");
    await act(async () => root.unmount());
  });
  it("adds an issue checkpoint while creating an issue", async () => {
    const onCreate = vi.fn(async (_projectId: string, _input: CreateIssueInput) => undefined);
    const root = createRoot(container);
    await act(async () => root.render(<CreateIssueDialog {...projectProps} isSubmitting={false} onClose={() => undefined} onCreate={onCreate} workflow={demoDashboard.settings.workflow} workflowProjectId="project-1" />));
    await act(async () => {
      container.querySelector<HTMLButtonElement>(".issue-checkpoint-trigger")?.dispatchEvent(new MouseEvent("pointerdown", {
        bubbles: true,
        button: 0
      }));
    });
    const option = Array.from(document.body.querySelectorAll<HTMLElement>('[role="menuitemcheckbox"]:not([data-disabled])')).find(item => item.getAttribute("data-state") === "unchecked");
    expect(option).toBeDefined();
    await act(async () => option?.click());
    const title = container.querySelector<HTMLInputElement>(".issue-title-input");
    await act(async () => {
      Object.getOwnPropertyDescriptor(HTMLInputElement.prototype, "value")?.set?.call(title, "Checkpoint issue");
      title?.dispatchEvent(new Event("input", {
        bubbles: true
      }));
      container.querySelector<HTMLFormElement>("form")?.requestSubmit();
  ...
  it("submits Full Auto and clears issue-specific checkpoints", async () => {
    ...
    const checkpoint = Array.from(document.body.querySelectorAll<HTMLElement>('[role="menuitemcheckbox"]:not([data-disabled])')).find(item => item.getAttribute("data-state") === "unchecked");
    await act(async () => checkpoint?.click());
    const fullAuto = container.querySelector<HTMLInputElement>(".issue-full-auto-toggle input");
    await act(async () => fullAuto?.click());
    expect(fullAuto?.checked).toBe(true);
    expect(container.querySelector<HTMLButtonElement>(".issue-checkpoint-trigger")?.disabled).toBe(true);
    const title = container.querySelector<HTMLInputElement>(".issue-title-input");
    await act(async () => {
      Object.getOwnPropertyDescriptor(HTMLInputElement.prototype, "value")?.set?.call(title, "Full Auto issue");
      title?.dispatchEvent(new Event("input", {
        bubbles: true
      }));
      container.querySelector<HTMLFormElement>("form")?.requestSubmit();
