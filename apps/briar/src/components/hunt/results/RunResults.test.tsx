...
describe("RunResults", () => {
  ...
  it("prioritizes verified deployment evidence for completed-issue manual QA", async () => {
    const completedRun: HuntRun = {
      ...
      workflow: {
        ...demoDashboard.runs[0].workflow,
        stages: [...demoDashboard.runs[0].workflow.stages, {
          id: "production_qa",
          label: "Production QA",
          required: true,
    ...
    await act(async () => {
      root.render(<TooltipProvider>
        ...
        })]} onMove={async () => undefined} onRetry={async () => undefined} onSendIssueMessage={async () => {
          throw new Error("not implemented in this test");
        }} run={completedRun} />
        </TooltipProvider>);
    ...
    expect(guide?.querySelector<HTMLAnchorElement>("a")?.getAttribute("href")).toBe("https://qa.example.com/result/2");
