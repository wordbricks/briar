...
import { loadExecutionWorkerSettings } from "../lib/workspace-worker-settings";
import {
...
const readinessTone = (
  readiness: WorkspaceExecutionWorker["bindings"][number]["readiness"],
) =>
  readiness === "available"
    ? "success"
    : readiness === "busy"
...
const workerStateTone = (state: WorkspaceExecutionWorker["state"]) =>
  state === "online" ? "success" : state === "stale" ? "warning" : "outline";

export function WorkspaceWorkersSettings({
  connectedTeamIds,
  workspace,
  projects,
  token,
  ...
  const connectedWorkspaceProjects = useMemo(
    () =>
  ...
  const [workers, setWorkers] = useState<WorkspaceExecutionWorker[]>([]);
  ...
  const [deleteCandidate, setDeleteCandidate] =
    useState<WorkspaceExecutionWorker | null>(null);
    ...
    try {
      const [remote, local] = await loadExecutionWorkerSettings({
        syncLabels: desktop
          ...
          : undefined,
        loadRemote: () =>
          loadWorkspaceExecutionWorkers(token, workspace.id),
        loadLocal: () =>
          desktop
            ? commands.inspectExecutionWorkers(
                connectedWorkspaceProjects.map((project) => project.id),
              )
            : Promise.resolve([]),
      });
  ...
  }, [connectedWorkspaceProjects, desktop, workspace.id, token]);
  ...
  const deleteDevice = async () => {
    ...
    try {
      ...
      await deleteWorkspaceExecutionWorker(
        token,
        workspace.id,
        device.deviceId,
      );
  ...
  const updateConcurrency = async (
    device: WorkspaceExecutionWorker,
    value: string,
  ) => {
    setSavingDeviceId(device.deviceId);
    setError(null);
    try {
      await updateWorkspaceExecutionWorkerConcurrency(
        token,
        workspace.id,
        device.deviceId,
        Number(value),
  ...
  const requestUpdate = async (worker: WorkspaceExecutionWorker) => {
  ...
  const editIcon = (worker: WorkspaceExecutionWorker) => {
    setEditingIconDeviceId(worker.deviceId);
    setIconDraft(worker.icon ?? null);
    setError(null);
  };

  const saveIcon = async (worker: WorkspaceExecutionWorker) => {
    if (iconDraft?.type === "emoji" && !isWorkerEmoji(iconDraft.value)) {
      setError(t("workspace.workerIconEmojiInvalid"));
      return;
    }
    setSavingDeviceId(worker.deviceId);
    setError(null);
    try {
      await updateWorkspaceExecutionWorkerIcon(
        token,
        workspace.id,
        worker.deviceId,
        iconDraft,
  ...
  const selectLogo = async (file: File | undefined) => {
    ...
    try {
      ...
      setError(t("workspace.workerIconUploadFailed"));
    } finally {
  ...
  return (
    <>
      <SettingsPageHeader
        ...
        description={t("workspace.workersDescription")}
        title={t("workspace.workers")}
      />
          ...
          <div className="flex min-w-0 items-center gap-3">
            ...
            <div className="min-w-0">
              <Typography as="h2" variant="bodyLg">
                {t("workspace.thisComputer")}
              </Typography>
              <Typography tone="muted" variant="caption">
                {desktop
                  ? (currentDevice?.label ??
                    t("workspace.thisComputerNotRegistered"))
                  : t("workspace.workersDesktopOnly")}
              </Typography>
            </div>
          </div>
          <Button
                ...
                >
                  <div className="min-w-0">
                    ...
                    <Typography tone="muted" variant="micro">
                      {connected
                        ? enabled
                          ? t("workspace.workerProjectEnabled")
                          : t("workspace.workerProjectAvailable")
                        : t("workspace.workerProjectRepositoryRequired")}
                    </Typography>
                  </div>
                  <Switch
                    aria-label={t("workspace.workerProjectToggle", {
                      name: project.name,
                    })}
                    checked={enabled}
                    disabled={
        ...
        {loading && workers.length === 0 ? (
          ...
          <div className="overflow-hidden rounded-xl border border-border bg-card shadow-xs">
            {workers.map((worker) => {
              ...
              const capacityLabel = t("workspace.workerCapacity", {
                active: activeSlots,
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-3">
                    ...
                    {mayManage ? (
                      <div className="ml-auto flex items-center gap-1.5">
                        ...
                        <Button
                          aria-label={t("workspace.workerUpdate", {
                            name: worker.label,
                          })}
                          disabled={
                        ...
                        <Button
                          aria-label={t("workspace.workerIconEdit", {
                            name: worker.label,
                          })}
                        ...
                        <SelectMenu
                          disabled={savingDeviceId === worker.deviceId}
                          label={t("workspace.workerConcurrency")}
                          onValueChange={(value) =>
                            void updateConcurrency(worker, value)
                          }
                          options={Array.from({ length: 16 }, (_, index) => ({
                            label: t("workspace.workerConcurrencyValue", {
                              count: index + 1,
                            }),
                        ...
                        />
                        <Button
                          aria-label={t("workspace.workersDelete", {
                            name: worker.label,
                          })}
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          <Input
                            ...
                            placeholder={t(
                              "workspace.workerIconEmojiPlaceholder",
                            )}
                            value={
                        ...
                        <label className="inline-flex min-h-9 cursor-pointer items-center gap-2 rounded-md border border-border bg-card px-3 text-xs font-medium hover:bg-accent">
                          ...
                          {t(
                            iconDraft?.type === "image"
                              ? "workspace.workerIconReplaceLogo"
                              : "workspace.workerIconUploadLogo",
                          )}
                          <input
      ...
      >
        <DialogContent className="sm:max-w-md" showClose={!savingDeviceId}>
          <DialogHeader>
            ...
            <DialogDescription>
              {deleteCandidate
                ? t("workspace.workersDeleteConfirm", {
                    name: deleteCandidate.label,
                  })
                : null}
            </DialogDescription>
          ...
          <DialogFooter>
            ...
            >
              ...
              {t("workspace.workersDeleteAction")}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
