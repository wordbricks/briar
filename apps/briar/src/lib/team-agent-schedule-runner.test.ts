...
import {
  executeClaimedTeamAgentSchedule,
  pollTeamAgentSchedulesOnce,
...
const dependencies = (
  overrides: Partial<TeamAgentScheduleRunnerDependencies> = {},
): TeamAgentScheduleRunnerDependencies => ({
...
describe("project agent schedule runner", () => {
  it("executes a claim and records the provider result", async () => {
    ...
    await executeClaimedTeamAgentSchedule(current, run);

    expect(current.execute).toHaveBeenCalledWith(run);
    expect(current.complete).toHaveBeenCalledWith(run.teamId, run.id, {
  ...
  it("claims all connected projects with one batched request", async () => {
    ...
    await pollTeamAgentSchedulesOnce(current, ["project-a", "project-b"]);
