...
export const requiredMessage = <T>(
  value: T | undefined,
  field: string,
): T => {
  if (value === undefined) throw new Error(`${field} is missing`);
...
export const optionalTimestamp = (
  value: Timestamp | undefined,
): string | null => value === undefined ? null : timestampDate(value).toISOString();
...
export const optionalAgentProviderFromProto = (
  ...
  value === undefined ? null : agentProviderFromProto(value);

export const agentProviderFromString = (
...
export const projectRoleFromProto = (
  value: ProtoProjectRole,
): Project["role"] => {
  switch (value) {
...
const structuredImpact = (
  value: StructuredRunResult_Impact,
): StructuredAgentResult["impact"] => {
  switch (value) {
...
): StructuredAgentResult | null => value === undefined
  ? null
  : {
...
export const issueAttachmentFromProto = (
  value: IssueAttachmentMessage,
): IssueAttachment => ({
  id: value.id,
...
export const messageAuthorFromProto = (
  value: MessageAuthorMessage,
): IssueMessageAuthor => ({
  id: value.id ?? null,
  agentId: value.agentId ?? null,
  name: value.name,
  image: value.image ?? null,
  provider: agentProviderFromString(value.provider),
});

export const resultReviewFromProto = (
  value: ResultReviewMessage,
): IssueResultReview => ({
  userId: value.userId,
  name: value.name,
