...
import {
  createCachedTeamUsageSummaryLoader,
...
const run = (
  id: string,
  completedAt: string,
  overrides: Partial<TeamUsageSummaryRun> = {},
): TeamUsageSummaryRun => ({
  id,
  status: "completed",
  executionMetrics: {
...
describe("project usage summary", () => {
  it("defaults to the latest 14 complete UTC date keys", () => {
    ...
    expect(defaultTeamUsageDateRange(now)).toEqual({
      from: "2026-07-30",
      to: "2026-08-12",
    });
  });
  ...
  it("accepts an inclusive custom date range and rejects invalid dates", () => {
    ...
    expect(isTeamUsageDateRange(range)).toBe(true);
    expect(teamUsageSummaryWindow("day", now, range)).toEqual({
      startAt: Date.parse("2026-07-04T00:00:00.000Z"),
      endAt: Date.parse("2026-07-07T00:00:00.000Z"),
    });
    expect(isTeamUsageDateRange({ from: "2026-02-30", to: "2026-03-01" }))
      .toBe(false);
    expect(isTeamUsageDateRange({ from: "2026-03-02", to: "2026-03-01" }))
      .toBe(false);
  });
  ...
  it("limits timeline size while allowing a wider range at coarser intervals", () => {
    ...
    expect(isTeamUsageDateRange(range, "day")).toBe(false);
    expect(isTeamUsageDateRange(range, "week")).toBe(true);
    expect(() => teamUsageSummaryWindow("day", Date.now(), range))
  ...
  it("uses ledger rows without falling back to execution metrics", () => {
    const now = Date.parse("2026-08-12T12:00:00.000Z");
    const summary = summarizeTeamUsage([
  ...
  it("reuses cached and in-flight requests until forced or expired", async () => {
    ...
    const cachedLoad = createCachedTeamUsageSummaryLoader(load, {
      ttlMs: 500,
      now: () => currentTime,
    });
