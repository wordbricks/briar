...
describe("local project connection state", () => {
  ...
  ] as const)("maps %j to %s", (connectedTeamIds, projectId, expected) => {
    expect(localTeamConnectionState(connectedTeamIds, projectId)).toBe(
      expected,
    );
  });

  it("prioritizes local disconnection over stale server and readiness data", () => {
    expect(teamRepositoryDestination({
  ...
  it("ignores stale readiness unless the local connection is known", () => {
    ...
    expect(localTeamReadiness("unknown", staleReadiness)).toBeNull();
    expect(localTeamReadiness("disconnected", staleReadiness)).toBeNull();
    expect(localTeamReadiness("connected", staleReadiness)).toBe(
      staleReadiness,
    );
  });
});
...
describe("local readiness coordinator", () => {
  it("shares inventory while allowing different projects to finish", async () => {
    ...
    const loadConnectedTeamIds = vi.fn(() => inventory.promise);
    ...
    expect(loadConnectedTeamIds).toHaveBeenCalledOnce();
  ...
  it("lets only the latest same-project observation commit", async () => {
    ...
    let call = 0;
    const coordinator = createLocalProjectReadinessCoordinator({
      loadConnectedTeamIds: async () => ["project-a"],
      loadReadiness: async () => {
        call += 1;
        if (call === 1) {
          firstStarted();
  ...
  it("forces a fresh inventory after a connection commit", async () => {
    ...
    const loadConnectedTeamIds = vi.fn()
    ...
    await expect(stale).resolves.toMatchObject({
      status: "loaded",
      connectedTeamIds: ["project-old"],
    });
    await expect(committed).resolves.toMatchObject({
      status: "loaded",
      connectedTeamIds: ["project-new"],
    });
    expect(loadConnectedTeamIds).toHaveBeenCalledTimes(2);
  });
