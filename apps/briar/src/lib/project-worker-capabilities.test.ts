...
describe("project Worker capabilities", () => {
  it("tracks liveness, policy, and heartbeat capability changes", () => {
    ...
    expect(
      projectWorkerCapabilityCatalog([first, second]).codex.models.map(
        (modelCapability) => modelCapability.id,
      ),
    ).toEqual(["model-first", "model-second"]);
    expect(
      projectWorkerCapabilityCatalog([first, second], allowedOnly).codex.models
        .map((modelCapability) => modelCapability.id),
    ).toEqual(["model-first"]);

    const expired = { ...first, state: "stale" as const, readiness: "offline" as const };
    expect(projectWorkerProviders([expired])).toEqual([]);
    expect(projectWorkerCapabilityCatalog([expired]).codex.models).toEqual([]);
    ...
    expect(
      projectWorkerCapabilityCatalog([changed]).codex.models.map(
        (modelCapability) => modelCapability.id,
      ),
