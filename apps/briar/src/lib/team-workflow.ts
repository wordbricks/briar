...
import {
  chatWithTeamLlm,
  type JsonSchema,
  type TeamLlmChatInput,
  type TeamLlmProgress,
} from "./team-llm";

export type TeamWorkflowChat = typeof chatWithTeamLlm;
...
export class TeamWorkflowGenerationError extends Schema.TaggedError<TeamWorkflowGenerationError>()(
...
) => new TeamWorkflowGenerationError({ phase, message, issues });
...
const repairWorkflowRequest = (error: TeamWorkflowGenerationError) => {
...
const requestGeneratedWorkflow = async (
  ...
  chat: TeamWorkflowChat,
): Promise<AutoHuntWorkflow> => {
  const response = await chat(input);
  try {
    return parseGeneratedWorkflow(response.message);
  } catch (error) {
    if (!Schema.is(TeamWorkflowGenerationError)(error)) throw error;
...
export const makeTeamWorkflowGenerator = (chat: TeamWorkflowChat) =>
  async (
    projectId: string,
...
export const generateTeamWorkflow = makeTeamWorkflowGenerator(
  chatWithTeamLlm,
);

export async function analyzeTeamWorkflowRequirements(
  projectId: string,
  currentWorkflow: AutoHuntWorkflow,
  onProgress?: (progress: TeamLlmProgress) => void,
): Promise<AutoHuntWorkflow> {
  const response = await chatWithTeamLlm({
    projectId,
...
export async function reviseTeamWorkflow(
  projectId: string,
  currentWorkflow: AutoHuntWorkflow,
