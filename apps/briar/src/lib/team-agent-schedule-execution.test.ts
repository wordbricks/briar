...
import {
  ...
  type TeamAgentScheduleExecutionDependencies,
} from "./team-agent-schedule-execution";
...
const dependencies = (): TeamAgentScheduleExecutionDependencies => ({
...
describe("scheduled project agent execution", () => {
  it("lets the Agent choose from its Skill roster with Agent-level runtime", async () => {
    ...
    run.agent.skills = [{
      ...
      description: "Use for signed desktop releases.",
      body: "Publish the signed desktop release.",
      provider: "claude",
    ...
    await executeScheduledTeamAgent(current, "token", run);
  ...
  it("runs a schedule through the same saved-Agent turn as a direct invocation", async () => {
    ...
    await expect(
      executeScheduledTeamAgent(current, "token", scheduledRun()),
  ...
  it("records a scheduled Agent session from start through completion", async () => {
    ...
    await executeScheduledTeamAgent(current, "token", scheduledRun());
