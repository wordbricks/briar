...
export const defaultTeamLlmSettings: TeamLlmSettings = {
  provider: "codex",
  model: null,
  effort: null,
...
export const defaultTeamSandboxSettings: ProjectSandboxSettings = {
  fullAccess: true,
};

export type TeamLlmChatInput = {
  projectId: string;
  message: string;
...
type TeamAgentRunSnapshotSource = {
  id: string;
  sourceKey: string;
  title: string;
  status: string;
...
export function teamAgentRunSnapshots(
  runs: readonly TeamAgentRunSnapshotSource[],
...
export const teamLlmChatRequest = (
  input: TeamLlmChatInput,
...
export const teamAgentRunRequest = (
 ...
 */
export async function chatWithTeamLlm(
  input: TeamLlmChatInput,
  ...
  try {
    return await commands.projectLlmChat(
      ...
      teamLlmChatRequest(input, progressId),
    );
  } finally {
    unlisten?.();
  }
...
export async function runTeamAgent(
  ...
  return commands.runProjectAgent(
    input.projectId,
    teamAgentRunRequest(input),
  );
}

export async function stopTeamAgentSession(
  sessionId: string,
): Promise<boolean> {
...
export async function loadTeamLlmSettings(
  projectId: string,
): Promise<TeamLlmSettings> {
  if (!isTauri()) return defaultTeamLlmSettings;
...
export async function loadTeamSandboxSettings(
  projectId: string,
): Promise<ProjectSandboxSettings> {
  if (!isTauri()) return defaultTeamSandboxSettings;
  return commands.loadProjectSandboxSettings(projectId);
}

export async function updateTeamSandboxSettings(
  projectId: string,
