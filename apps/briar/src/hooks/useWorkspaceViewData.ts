...
/*
  The reads the workspace-wide views make, and the one navigation they need.
  ...
  take them — the usage report, the lobby's per-team cards, the workspace's
...
export interface WorkspaceViewData {
  /** Every agent run this workspace billed in the last 90 days. */
   ...
   */
  readonly loadWorkspaceTeamDashboard: MyIssuesDashboardLoader;
...
export function useWorkspaceViewData(): WorkspaceViewData {
  ...
  const loadUsageReport = useCallback(async () => {
    ...
    return loadAgentUsageReport(token, activeWorkspaceId, 90);
  ...
  const loadWorkspaceTeamDashboard = useCallback(
    (teamId: string, signal: AbortSignal) => {
      /*
        ...
        server to ask, showing its own board on the workspace pages.
  ...
  const openWorkspaceIssue = useCallback(
