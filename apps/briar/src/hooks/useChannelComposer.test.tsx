...
Object.assign(globalThis, { IS_REACT_ACT_ENVIRONMENT: true });
  ...
  it("localizes candidates and picks the active mention with the keyboard", async () => {
    ...
    expect(container.textContent).toContain("Helper · Workspace agent");
  ...
  */
  ...
  it.each(["drop", "paste", "change"])("restores image and PDF MIME aliases through %s", async (kind) => {
    const onSend = vi.fn<OnSend>();
    const { cleanup, container } = await renderHarness({ onSend });
    const files = [new File(["photo"], "photo.jpg", { type: "image/jpg" }), new File(["%PDF-1.7"], "brief.pdf", { type: "application/x-pdf" })];
    const event = new Event(kind, { bubbles: true, cancelable: true });
    if (kind === "change") {
      const input = container.querySelector<HTMLInputElement>('input[type="file"]')!;
      Object.defineProperty(input, "files", { value: files });
    ...
    const sent = onSend.mock.calls[0]![2];
    expect(sent.map(file => [file.name, file.type, file.size])).toEqual([
      ["photo.jpg", "image/jpeg", files[0]!.size], ["brief.pdf", "application/pdf", files[1]!.size],
    ]);
    await cleanup();
  });
