...
import {
  ...
  type WorkspaceAgentContextClient,
...
const contextClient = (input: {
  getManifest?: WorkspaceAgentContextClient["getManifest"];
  lookup?: WorkspaceAgentContextClient["lookup"];
}): WorkspaceAgentContextClient => ({
...
describe("Workspace Agent context downloader", () => {
  ...
  it("removes crashed-process workspaces while preserving live owners", async () => {
    ...
    await prepareWorkspaceAgentWorkspace(deadWorkspace, 101);
    await prepareWorkspaceAgentWorkspace(liveWorkspace, 202);
    await prepareWorkspaceAgentWorkspace(retainedWorkspace, 303, {
      reuse: true,
  ...
  it("downloads only the revision manifest and reuses an unchanged cached index", async () => {
    ...
    const firstGetManifest = vi.fn<
      WorkspaceAgentContextClient["getManifest"]
    >(async (request) => {
    ...
    const first = await downloadWorkspaceAgentContextManifest({
    ...
    const second = await downloadWorkspaceAgentContextManifest({
      apiUrl: "https://briar.example",
      workerToken: "worker-token",
      workspaceId,
      workId,
  ...
  it("hydrates selected detail files and atomically updates the manifest", async () => {
    ...
    const lookup = vi.fn<WorkspaceAgentContextClient["lookup"]>(
      async (input) => {
    ...
    const prepared = await downloadWorkspaceAgentContextManifest({
      apiUrl: "https://briar.example",
      workerToken: "worker-token",
      workspaceId,
      workId,
    ...
    const repeated = await hydrateWorkspaceAgentContext({
      apiUrl: "https://briar.example",
      workerToken: "worker-token",
      workspaceId,
      workId,
