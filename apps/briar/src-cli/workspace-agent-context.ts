...
import {
  ...
  type WorkspaceAgentContextManifest as WorkspaceAgentContextManifest} from "@briar/contracts/gen/briar/worker/v1/workspace_agent_context_pb";
import type { Client } from "@connectrpc/connect";
import {
  decodeWorkspaceAgentContextManifest,
  type WorkspaceAgentContextLookupRequest,
  type WorkspaceAgentContextManifest as WorkspaceAgentContextIndexManifest,
} from "../src/lib/workspace-agent-context-contract";
import { createAuthenticatedConnectClient } from "./connect-client";

export const organizationAgentContextDirectoryName =
  ".briar-workspace-context";
...
export async function cleanupWorkspaceAgentContext(workspacePath: string) {
...
export async function prepareWorkspaceAgentWorkspace(
...
export async function cleanupOrphanedWorkspaceAgentWorkspaces(input: {
...
const contextFilePath = (directory: string, relativePath: string) => {
  ...
  if (!target.startsWith(`${root}${sep}`)) {
    throw new Error("Workspace Agent context path escaped its workspace");
  }
  return target;
};

type WorkspaceContextManifestCacheEntry = {
  revision: string;
  projects: WorkspaceAgentContextIndexManifest["projects"];
};

const organizationContextManifestCache = new Map<
  string,
  WorkspaceContextManifestCacheEntry
>();
const maxWorkspaceContextManifestCacheEntries = 16;

const rememberWorkspaceContextManifest = (
  cacheKey: string,
  entry: WorkspaceContextManifestCacheEntry,
) => {
  ...
  while (
    organizationContextManifestCache.size >
      maxWorkspaceContextManifestCacheEntries
  ) {
...
export type WorkspaceAgentContextClient = Pick<
...
const organizationContextClient = (input: {
  apiUrl: string;
  workerToken: string;
  client?: WorkspaceAgentContextClient;
...
const required = <T>(value: T | undefined, field: string): T => {
  if (value === undefined) {
    throw new Error(`Workspace Agent context omitted ${field}`);
  }
  return value;
};
...
const isoTimestamp = (
  ...
  if (Number.isNaN(date.getTime())) {
    throw new Error(`Workspace Agent context returned invalid ${field}`);
  }
...
const manifestFromProto = (
  manifest: WorkspaceAgentContextManifest,
): WorkspaceAgentContextIndexManifest =>
  decodeWorkspaceAgentContextManifest({
    schemaVersion: 2,
...
/**
 * Prepares only the lightweight workspace index. Detailed context remains
 * server-side until hydrateWorkspaceAgentContext is called with a bounded,
 * model-selected request.
 */
export async function downloadWorkspaceAgentContextManifest(input: {
  apiUrl: string;
  workerToken: string;
  workspaceId: string;
  workId: string;
  ...
  const cacheKey = `${input.apiUrl.replace(/\/$/u, "")}:${input.workspaceId}`;
  const cached = organizationContextManifestCache.get(cacheKey);
  const client = organizationContextClient(input);
  await cleanupWorkspaceAgentContext(input.workspacePath);
  ...
  try {
    ...
    let manifest: WorkspaceAgentContextIndexManifest;
    if (response.result.case === "unchanged") {
      if (!cached) {
        throw new Error("Workspace Agent manifest cache is missing");
      }
      ...
      if (unchanged.revision !== cached.revision) {
        throw new Error("Workspace Agent manifest cache revision changed");
      }
      manifest = decodeWorkspaceAgentContextManifest({
        schemaVersion: 2,
      ...
      rememberWorkspaceContextManifest(cacheKey, {
        revision: manifest.revision,
        projects: manifest.projects,
      });
    } else {
      throw new Error("Workspace Agent manifest response omitted its result");
    }
    if (
      ...
      throw new Error("Workspace Agent manifest scope changed");
    }
...
export async function hydrateWorkspaceAgentContext(input: {
  apiUrl: string;
  workerToken: string;
  workspaceId: string;
  workId: string;
  ...
  snapshotAt: string;
  workspacePath: string;
  requests: WorkspaceAgentContextLookupRequest[];
  signal?: AbortSignal;
  client?: WorkspaceAgentContextClient;
  ...
  const directory = organizationAgentContextDirectory(input.workspacePath);
  const manifestPath = join(directory, "manifest.json");
  const manifest = decodeWorkspaceAgentContextManifest(
  ...
  if (
    ...
    throw new Error("Workspace Agent lookup scope changed");
  }
  if (
    ...
    throw new Error("Workspace Agent lookup response did not match its request");
  }
  ...
  for (const result of results) {
    ...
    if (contextBytes > (input.maxContextBytes ?? defaultMaxContextBytes)) {
      throw new Error("Workspace Agent context exceeds the local size limit");
    }
  ...
  const nextManifest = decodeWorkspaceAgentContextManifest({
    ...manifest,
