...
describe("early DM acknowledgement", () => {
  ...
  it("publishes the selected emoji with no preceding fallback while the body is still pending", async () => {
  ...
  it("falls back on timeout and ignores late success", async () => {
    vi.useFakeTimers();
  ...
  it("contains selection and publication errors and cancels on claim shutdown", async () => {
    ...
    const cancel = startDmAcknowledgement({ select: async () => '{"emoji":"🎉"}',
