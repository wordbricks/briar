...
describe("exact-SHA merge-group foundation", () => {
  ...
  it.each([
    ...
    "apps/briar/worker/src/test-helpers/d1.ts",
    "apps/briar/src/test/setup.ts",
  ...
  it("allows ordinary application and test source changes", () => {
    expect(() => assertTrustedCandidateConfiguration([
      "apps/briar/src/components/QueuePanel.tsx",
      "apps/briar/src/components/QueuePanel.test.tsx",
      "apps/briar/worker/src/queue.ts",
      "apps/briar/src-tauri/src/lib.rs",
    ])).not.toThrow();
  });

  it("keeps every fixed phase in the trusted wrapper and out of candidate-writable PATH", async () => {
    const wrapper = await readFile(
      new URL("../../../scripts/ci-merge-group.sh", import.meta.url),
      "utf8",
    );
