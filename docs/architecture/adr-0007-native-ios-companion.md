...
Android Tauri 앱은 계속 같은 사용자 기능을 제공하며, 공유 TypeScript client와 operation descriptor가 Android 경계도 함께 검증한다. 기존 Android 빌드는 명시적인 모바일 CI에서 확인한다. 이후 모바일 기능 PR은 기능 동등성 표의 Android와 iOS 열을 함께 갱신해야 한다.
