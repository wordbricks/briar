...
상태: 구현 진행 중. 저장·관리 기반은 PR #1497, 검색은 PR #1499,
DM 실행 연결은 PR #1500으로 main에 반영했다. 자동 학습은 네 번째 PR에서
구현 중이다. 검색·자동 학습은 검증 후 별도로 활성화하며, 배포 전이다.
구현 범위와 검증 기록은 [진행 문서](dm-memory-delivery.md)에 남긴다.
...
코드 확인 기준: `a8f612051413ec7a997c5ce96b3e90effa370e11`. 이 문서의
현재 동작 설명은 해당 워크트리 기준이며, 프로덕션 배포 상태를 뜻하지 않는다.
...
| 검색 방식 | 벡터 검색. FTS5, BM25, 키워드 혼합 검색, 별도 reranker는 v1 제외 |
...
이 문서는 구현 기준을 제안한다. 자동 학습 활성화, 모델 비용 지출, 저장된 DM의
백필, 외부 배포를 지금 승인하거나 실행하는 문서는 아니다.
...
6. 평가로 정한 최소 관련성 기준 아래의 후보는 제외하고 유사도 순으로 정렬한다.
   동점은 사용자 명시 근거, 갱신일, 안정적인 ID 순으로 정한다.
7. `max_results`와 응답 분량 한도 안에서 원본 발췌·근거를 반환한다.
8. 전송 직전에 revocationEpoch를 다시 확인한다. 변경됐다면 본문을 보내지 않는다.
...
이 문서 작성으로 배포·기존 DM 백필을 실행하지 않는다.
