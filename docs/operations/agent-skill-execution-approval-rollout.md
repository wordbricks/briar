...
4. Apply migrations with `bun run d1:migrate:remote`.
5. Deploy the Worker from the same verified SHA with
   `bun run worker:deploy`.
6. While the gate remains closed to normal traffic, smoke-test all three
   origins:
...
7. For every origin, verify that the natural-language match creates one pending
...
8. Approve with one exact, available, policy-allowed Worker. Verify one linked
...
9. Set a Worker device's maximum concurrency to one. Race two task claims and
...
10. Retry approval with the same member and Worker and verify the existing
...
11. Before reopening the gate, verify pending cards are invalidated by relevant
...
12. Remove or disable each approved authority in a disposable fixture: Agent,
...
13. Archive a completed approved session, repeat the same-member/same-Worker
