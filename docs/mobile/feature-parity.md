...
| DM 기억 저장·관리 | 제공 | 제공 | 같은 소유자 전용 API로 저장·편집·삭제·ZIP 내보내기, 분류·근거·색인 상태 표시. DM별 회상·자동 학습 opt-in과 연결된 Agent provider·실패 상태를 같은 계약으로 표시 |
...
| 이슈 계층·관련 이슈·실행 의존성 | 제공 | 제공 | 부모 이동, 하위 이슈 생성·연결, 대칭 관련 이슈 추가·제거, 연결 이슈 열기를 공유한다. 계층과 관련 이슈는 실행을 막지 않고 선행 이슈 DAG만 실행 준비 상태에 반영 |
| 실행 설정 | 제공 | 제공 | provider/model/effort 설정을 dashboard 및 공통 쓰기 API로 사용 |
