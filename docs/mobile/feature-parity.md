...
| DM 대화 헤더 상대 표시·프로필 | 제공 | 제공 | 제공 | 뒤로가기 옆에 상대 아바타·이름을 두고, 1:1은 탭하면 프로필 모달, 그룹은 참여자 드롭다운 후 프로필 모달을 연다 |
...
| 메시지 텍스트 범위 선택·복사 | 제공 | 제공 | 제공 | 네이티브 iOS는 편집 불가 UITextView, Android/Tauri는 메시지 본문에 `user-select: text`를 사용한다 |
| 채널·DM 스레드 진입 | 제공 | 제공 | 제공 | 채널 루트에 답글이 있으면 답글 수 요약을 보여주고 탭하면 스레드를 연다. 길게 누르면 아래에서 Start a thread 액션을 연다. DM은 답글을 인라인으로 이어서 보여 주고 스레드 요약은 쓰지 않는다 |
