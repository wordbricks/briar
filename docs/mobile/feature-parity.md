...
| 메시지 텍스트 범위 선택·복사 | 제공 | 제공 | 제공 | 길게 눌러 범위를 지정한 뒤 복사. 네이티브 iOS는 편집 불가 UITextView, Android/Tauri는 메시지 본문에 `user-select: text`. 채널 메시지는 overflow 메뉴에서 Copy link / Copy message를 제공하며, iOS는 길게 눌러 같은 동작을 제공한다 |
