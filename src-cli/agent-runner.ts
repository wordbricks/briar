...
export function detachedAgentPrompt(input: {
  ...
  return [
    ...
    "If a stage command returns `delegated`, repository delivery has moved to the durable merge-batch coordinator and the issue claim has been released. Stop immediately; the original run will resume at `merged` only after GitHub merges its immutable pull request through the native queue.",
